...
let query_count_lwt db sql =
  ...
  | Ok stream ->
    Lwt_stream.to_list stream >|= fun rows ->
    List.length rows
  ...
  let make_reader _id =
    let rec loop seen i =
      if i >= n_reads_each then Lwt.return ()
        ...
        query_count_lwt db "SELECT n FROM t" >>= fun c ->
        if c < seen then
          Alcotest.failf "reader saw count decrease: %d → %d" seen c;
        Lwt.pause () >>= fun () ->
        loop c (i + 1)
    in
    loop 0 0
...
let test_writer_yields_between_inserts () =
  ...
  let reader =
    let rec loop i acc =
      if i >= 50 then Lwt.return acc
      else
        query_count_lwt db "SELECT id FROM t WHERE n = 7" >>= fun c ->
        Lwt.pause () >>= fun () -> loop (i + 1) (c :: acc)
    in
    loop 0 []
  in
  ...
  let expected = List.length (List.filter (fun i -> i mod 50 = 7) (List.init n_writes Fun.id)) in
...
let test_mixed_writer_with_aggregating_readers () =
  ...
  let make_reader _ =
    let rec loop i =
      if i >= 80 then Lwt.return ()
      else
        select_first_int_lwt db "SELECT SUM(n) FROM t" >>= fun s ->
...
let test_explicit_txn_isolation () =
  ...
  let open Lwt.Infix in
  let writer =
    exec_lwt db "BEGIN" >>= fun () ->
    let rec loop i =
      if i >= 30 then Lwt.return ()
  ...
  let rec mono prev = function
    ...
    | c :: rest ->
      if c < prev then
        Alcotest.failf "reader saw count regress: %d → %d" prev c;
      mono c rest
  in
  mono 0 observed;
  let final = run (query_count_lwt db "SELECT n FROM t") in
...
let test_file_backend_concurrent () =
  ...
  let db = match run (Db.open_file ~path) with
    | Ok d -> d
  ...
  let reader =
    let rec loop i seen =
      if i >= 60 then Lwt.return seen
      else
        query_count_lwt db "SELECT n FROM t" >>= fun c ->
        if c < seen then Alcotest.failf "file-backend reader saw count drop %d → %d" seen c;
...
let () =
  Alcotest.run "multifiber_stress" [
    "concurrency", [
      Alcotest.test_case "many readers + one writer" `Slow test_many_readers_one_writer;
      Alcotest.test_case "writer yields between inserts" `Slow test_writer_yields_between_inserts;
