...
let parse_stmt sql =
  ...
  try Some (Parser.stmt_eof Lexer.token lexbuf)
  with _ -> None
      ...
      if orders_idx then
        let* r =
          Cat.create_index cat ~name:"idx_orders_uid"
            ~table:"orders" ~columns:["uid"] ~unique:false
            ~expr_flags:[false] ~where_sql:None in
...
let parse_left_outer_join () =
  match parse_stmt "SELECT * FROM users LEFT OUTER JOIN orders ON users.id = orders.uid" with
  | Some (Ast.S_select { joins = [j]; _ }) ->
    (match j.kind with
     | Ast.Left -> ()
...
let inner_join_matching () =
  ...
  let rows = query_ok db
    "SELECT * FROM users INNER JOIN orders ON users.id = orders.uid" in
  ...
  List.iter (fun r ->
    ...
    (match r.(0), r.(2) with
     | Row.V_int a, Row.V_int b ->
       Alcotest.(check int64) "id == uid" a b
...
let left_join_preserves_outer () =
  ...
  exec db "INSERT INTO orders (uid, item) VALUES (1, 'book')";
  let rows = query_ok db
    "SELECT * FROM users LEFT JOIN orders ON users.id = orders.uid" in
  Alcotest.(check int) "two rows (alice matched, bob unmatched)" 2 (List.length rows);
  (* Find bob: should have V_null in cols 2,3 *)
  let bob_row =
    List.find (fun r -> match r.(0) with Row.V_int 2L -> true | _ -> false) rows
  in
...
let join_with_order_by () =
  ...
  let rows = query_ok db
    "SELECT * FROM users INNER JOIN orders ON users.id = orders.uid \
     ORDER BY item ASC" in
  Alcotest.(check int) "two rows" 2 (List.length rows);
  let items = List.map (fun r -> match r.(3) with
    | Row.V_text s -> s | _ -> "") rows in
  Alcotest.(check (list string)) "sorted ascending by item"
    ["apple"; "zebra"] items
...
let cartesian_inner us os =
  List.concat_map (fun u ->
    List.filter_map (fun (uid, item) ->
      if u = uid then Some (u, item) else None
    ) os
  ) us
...
let qcheck_inner_join_subset_cartesian =
  QCheck.Test.make
    ...
    QCheck.(pair
              ...
              (list_size Gen.(0 -- 8) (pair (1 -- 5) (1 -- 5)))) (* (uid, item) *)
    (fun (us, os) ->
       let db = fresh_db () in
       exec db "CREATE TABLE users (id INTEGER, name TEXT)";
       ...
       List.iter (fun u ->
         ...
         exec db sql) us;
       List.iter (fun (uid, item) ->
         let sql = Printf.sprintf
           "INSERT INTO orders (uid, item) VALUES (%d, %d)" uid item in
         exec db sql) os;
       let rows = query_ok db
...
let qcheck_left_join_ge_inner =
  QCheck.Test.make
    ...
    (fun (us, os) ->
       ...
       exec db "CREATE TABLE orders (uid INTEGER, item INTEGER)";
       List.iter (fun u ->
         exec db (Printf.sprintf "INSERT INTO users (id) VALUES (%d)" u)) us;
       List.iter (fun (uid, item) ->
         exec db (Printf.sprintf
           "INSERT INTO orders (uid, item) VALUES (%d, %d)" uid item)) os;
       let inner = query_ok db
         "SELECT * FROM users INNER JOIN orders ON users.id = orders.uid" in
       let left = query_ok db
         "SELECT * FROM users LEFT JOIN orders ON users.id = orders.uid" in
       (* LEFT JOIN size ≥ INNER JOIN size, and LEFT JOIN contains all
          left rows (each appears at least once, even with NULL right). *)
       List.length left >= List.length inner
       && List.length left >= List.length us)
...
let () =
  Alcotest.run "JOIN" [
    ...
    "sema_errors", [
      ...
      Alcotest.test_case "qualified column resolves" `Quick sema_qualified_column_resolves;
    ];
    "planner", [
      Alcotest.test_case "hash join when no index"   `Quick planner_picks_hash_join_no_index;
    ...
    "qcheck", (
      List.map QCheck_alcotest.to_alcotest [
        qcheck_inner_join_subset_cartesian;
        qcheck_left_join_ge_inner;
      ]
    );
  ]
