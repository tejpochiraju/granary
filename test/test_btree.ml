...
let test_get_empty () =
  ...
  Alcotest.(check bool) "get on empty tree returns Ok None" true (r = Ok None)
;;
...
let test_put_then_get () =
  ...
  Alcotest.(check bool) "get returns Some v" true (r = Ok (Some (b "v")))
;;
...
let test_put_update () =
  ...
  let r = run (Btree.get t (b "k")) in
  Alcotest.(check bool) "get returns updated value" true (r = Ok (Some (b "v2")))
;;
...
let test_del_missing () =
  ...
  Alcotest.(check int64) "root unchanged after no-op del" root_before (Btree.root_page t');
  let r = run (Btree.get t' (b "k")) in
...
let test_put_then_del () =
  ...
  Alcotest.(check int64)
    "root_page becomes 0L after deleting last key"
    0L
    (Btree.root_page t)
;;
...
let test_insert_10_random () =
  ...
  in
  let t =
    List.fold_left (fun t (k, v) -> ok_btree (run (Btree.put t (b k) (b v)))) t pairs
  in
  List.iter
    (fun (k, v) ->
       let r = run (Btree.get t (b k)) in
...
let test_cursor_5_keys_sorted () =
  ...
  let t =
    List.fold_left (fun t k -> ok_btree (run (Btree.put t (b k) (b (k ^ "v"))))) t keys
  in
  let c = ok_btree (run (Btree.cursor_open t)) in
  let rec collect acc =
...
let test_cursor_seek_found () =
  ...
  match run (Btree.cursor_next c) with
  | Ok (Some (k, v)) ->
    Alcotest.(check string) "first next returns matched key" "c" (Bytes.to_string k);
    Alcotest.(check string) "first next returns its value" "cv" (Bytes.to_string v)
...
let test_cursor_seek_between () =
  ...
  Alcotest.(check bool)
    "seek between keys returns Not_found_after"
    true
    (match r with
     | Ok (`Not_found_after _) -> true
     | _ -> false);
  match run (Btree.cursor_next c) with
  | Ok (Some (k, _)) ->
    Alcotest.(check string) "first next returns next key (c)" "c" (Bytes.to_string k)
...
let test_cursor_seek_past_end () =
  ...
  Alcotest.(check bool)
    "seek past end returns Not_found_after"
    true
    (match r with
     | Ok (`Not_found_after _) -> true
     | _ -> false);
  let r2 = run (Btree.cursor_next c) in
  Alcotest.(check bool) "cursor_next at end returns None" true (r2 = Ok None)
;;
...
let test_value_too_large () =
  ...
  | Ok t' ->
    ...
    Alcotest.(check bool)
      "value round-trips via overflow chain"
      true
      (v = Ok (Some big_value))
  | Error e ->
    Alcotest.failf "unexpected error storing 1025-byte value: %a" Btree.pp_error e
;;
...
let test_tree_corrupt_bad_page_kind () =
  ...
  let read_page, write_page, sync, resize = mock_callbacks mb in
  let pager =
    Pager.create ~read_page ~write_page ~sync ~resize ~n_pages:3L ~freelist:Freelist.empty
  in
...
let test_empty_branch_cursor () =
  ...
  let read_page, write_page, sync, resize = mock_callbacks mb in
  let pager =
    Pager.create ~read_page ~write_page ~sync ~resize ~n_pages:4L ~freelist:Freelist.empty
  in
  ...
  Page.write_common
    leaf_buf
    Page.{ kind = Page.Leaf; flags = 0; n_keys = 1; right_page = 0l; crc32 = 0l };
  Page.seal leaf_buf;
...
let test_pp_error_all_variants () =
  ...
  let s_pager_block =
    Format.asprintf "%a" pe1 (Btree.Pager_error (Pager.Block_error "x"))
  in
  let s_pager_corr =
    Format.asprintf "%a" pe1 (Btree.Pager_error (Pager.Corruption "c"))
  in
...
let failable_pager mb =
  let read_page ~page_id buf =
    if mb.fail_reads
    then Lwt.return_error "injected read error"
    else (
      match Hashtbl.find_opt mb.store page_id with
      | None ->
        Cstruct.memset buf 0;
  ...
  let write_page ~page_id buf =
    if mb.fail_writes
    then Lwt.return_error "injected write error"
    else (
      let bytes = Bytes.create Page.page_size in
      Cstruct.blit_to_bytes buf 0 bytes 0 Page.page_size;
      Hashtbl.replace mb.store page_id bytes;
  ...
  let resize ~n_pages =
    if mb.fail_resize
    then Lwt.return_error "injected resize error"
    else (
      mb.n_pages <- n_pages;
      Lwt.return_ok ())
  in
...
let test_btree_put_alloc_error () =
  ...
  | Error e -> Alcotest.failf "expected Pager_error Block_error, got: %a" Btree.pp_error e
...
let always_failing_pager () =
  ...
  let sync () = Lwt.return_ok () in
  let resize ~n_pages:_ = Lwt.return_ok () in
  Pager.create ~read_page ~write_page ~sync ~resize ~n_pages:100L ~freelist:Freelist.empty
;;
...
let test_cursor_next_read_error () =
  ...
  | Error (Btree.Pager_error _) -> ()
  | Error e ->
    Alcotest.failf "expected Pager_error on cursor_open, got: %a" Btree.pp_error e
...
let test_cursor_advance_next_leaf_error () =
  ...
  let drain_count = 72 in
  (* ~4 entries/leaf × 72 leaves *)
...
let key_gen =
  let open QCheck in
  Gen.(map (fun s -> Bytes.of_string s) (string_size (int_range 1 16)))
;;

let val_gen =
  let open QCheck in
  Gen.(map (fun s -> Bytes.of_string s) (string_size (int_range 0 32)))
;;
         ...
         (match collect [] with
          ...
          | Some keys ->
            let rec is_sorted = function
              | [] | [ _ ] -> true
              | a :: (b :: _ as rest) -> Bytes.compare a b < 0 && is_sorted rest
            in
            is_sorted keys))
;;
...
let prop_put_del =
  QCheck.Test.make
    ...
    (fun (kvs, target_k, target_v) ->
       ...
       in
       let t =
         match run (Btree.put t target_k target_v) with
         | Ok t -> t
         | Error _ -> t
       in
       let t =
         match run (Btree.del t target_k) with
         | Ok t -> t
         | Error _ -> t
       in
...
let () =
  let qcheck_tests =
    List.map
      ...
      [ prop_put_get; prop_cursor_sorted; prop_put_del; prop_root_nonzero_after_put ]
  in
  Alcotest.run
    "btree"
    [ ( "basic"
      , [ Alcotest.test_case "get on empty tree" `Quick test_get_empty
        ...
        ; Alcotest.test_case
            "tree_corrupt bad page kind"
            `Quick
            test_tree_corrupt_bad_page_kind
