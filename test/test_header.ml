...
type mock_block = { store : (int64, Bytes.t) Hashtbl.t }

let make_mock () = { store = Hashtbl.create 4 }
    ...
    (match run (Header.commit pager ~prev_header:prev ~new_state) with
     ...
     | Ok () ->
       (match run (Header.read_live pager) with
        ...
        | Ok h -> Alcotest.(check int64) "txn_id after commit = 1" 1L h.Header.txn_id))
;;
...
let test_multiple_commits () =
  ...
  | Ok () ->
    ...
    Alcotest.(check int64)
      "txn_id = 5 after 5 commits"
      (Int64.of_int n)
      !prev_h.Header.txn_id
;;
...
let test_corrupt_page1_fallback_to_page0 () =
  ...
  | Ok () ->
    ...
    (match run (Header.read_live pager2) with
     ...
     | Ok h -> Alcotest.(check int64) "falls back to page 0 (txn_id=2)" 2L h.Header.txn_id)
;;
  ...
  | Ok () ->
    ...
    (match run (Header.commit pager ~prev_header:zero_header ~new_state) with
     ...
     | Ok () ->
       ...
       Page.write_common
         buf
         { Page.kind = Page.Leaf; flags = 0; n_keys = 0; right_page = 0l; crc32 = 0l };
       Page.seal buf;
       let bytes = Bytes.create Page.page_size in
       Cstruct.blit_to_bytes buf 0 bytes 0 Page.page_size;
...
let test_pp_error_both_corrupt () =
  let s = Format.asprintf "%a" Header.pp_error Header.Both_headers_corrupt in
  Alcotest.(check bool)
    "Both_headers_corrupt"
    true
    (string_contains s "Both_headers_corrupt")
;;
...
let test_read_live_one_read_error () =
  ...
  let read_page ~page_id buf =
    if Int64.equal page_id 0L
    then Lwt.return_error "broken"
    else (
  ...
  | Error e -> Alcotest.failf "expected Ok with page 1, got: %a" Header.pp_error e
;;
...
let () =
  let qcheck_tests = List.map QCheck_alcotest.to_alcotest [ prop_commit_sequence ] in
  Alcotest.run
    "header"
    [ ( "basic"
      , [ Alcotest.test_case "init then read_live" `Quick test_init_then_read_live
        ; Alcotest.test_case
            "commit increments txn_id"
            `Quick
            test_commit_increments_txn_id
        ...
        ; Alcotest.test_case
            "corrupt page 0 falls back to page 1"
            `Quick
            test_corrupt_page0_fallback_to_page1
        ; Alcotest.test_case
            "corrupt page 1 falls back to page 0"
            `Quick
            test_corrupt_page1_fallback_to_page0
    ...
    ; ( "errors"
      , [ Alcotest.test_case "pp_error Io" `Quick test_pp_error_io
        ; Alcotest.test_case
            "pp_error Both_headers_corrupt"
            `Quick
            test_pp_error_both_corrupt
        ...
        ; Alcotest.test_case
            "read_live both read errors"
            `Quick
            test_read_live_both_read_errors
        ; Alcotest.test_case
            "read_live one read error"
            `Quick
            test_read_live_one_read_error
        ] )
    ; "qcheck", qcheck_tests
    ]
;;
