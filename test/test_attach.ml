...
let count_rows db sql =
  List.length (collect_query db sql)

let single_int db sql =
  match collect_query db sql with
  | [row] -> (match row.(0) with Db.V_int n -> Int64.to_int n | _ -> -1)
  | _ -> -1

let single_text db sql =
  match collect_query db sql with
  | [row] -> (match row.(0) with Db.V_text s -> s | _ -> "")
  | _ -> ""
...
let test_attach_detach_lifecycle () =
  ...
  Alcotest.(check string) "active is main by default" "main"
...
let test_isolation_between_schemas () =
  ...
  Alcotest.(check string) "switched to aux" "aux"
    (single_text db "PRAGMA active_database");
  exec db "CREATE TABLE t (n INTEGER)";
...
let test_detach_active_resets () =
  ...
  exec db "CREATE TABLE t (n INTEGER)";
  exec db "DETACH DATABASE aux";
  Alcotest.(check string) "active back to main after detach"
    "main"
    (single_text db "PRAGMA active_database");
  run (Db.close db);
  cleanup aux
...
let test_persistence_across_detach () =
  ...
  Alcotest.(check int) "rows survive detach/reattach"
    2 (single_int db "SELECT COUNT(*) FROM notes");
  Alcotest.(check string) "body persisted"
    "first"
...
let test_per_schema_transactions () =
  ...
  exec db "ROLLBACK";
  Alcotest.(check int) "aux rollback observed" 0
    (single_int db "SELECT COUNT(*) FROM a");
  exec db "PRAGMA active_database = 'main'";
  exec db "INSERT INTO m (n) VALUES (1)";
  Alcotest.(check int) "main unaffected" 1
    (single_int db "SELECT COUNT(*) FROM m");
  run (Db.close db);
  cleanup aux
...
let () =
  Alcotest.run "attach" [
    "lifecycle", [
      Alcotest.test_case "attach/detach + database_list" `Quick test_attach_detach_lifecycle;
      Alcotest.test_case "isolation between schemas" `Quick test_isolation_between_schemas;
      ...
      Alcotest.test_case "detach active schema resets routing" `Quick test_detach_active_resets;
      Alcotest.test_case "persistence across detach + reattach" `Quick test_persistence_across_detach;
