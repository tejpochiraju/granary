...
let test_run_dispatches_per_availability () =
  ...
  if Parallel.available () then
    Alcotest.(check int) "parallel branch ran" 1 !p
...
let () =
  Alcotest.run "parallel"
    [
      ( "gate",
        [
          Alcotest.test_case "run returns branch value" `Quick
            test_run_returns_branch_value;
          Alcotest.test_case "run dispatches per availability" `Quick
