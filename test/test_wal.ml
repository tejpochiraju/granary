...
let test_empty_open () =
  Lwt_main.run
    (let* _, w = fresh_wal () in
     ...
     Alcotest.(check (option int)) "no page in fresh WAL" None (Wal.find_page w 42L);
     Lwt.return_unit)
;;
...
let test_recovery_drops_partial_trailing_batch () =
  Lwt_main.run
    (let* d, w1 = fresh_wal () in
     ...
     Cstruct.BE.set_uint64 half 0 99L;
     (* page_id *)
     Cstruct.BE.set_uint64 half 8 1L;
     (* commit flag *)
     Cstruct.BE.set_uint64 half 16 0L;
     (* bogus checksum *)
     let frame_off =
       Int64.add (Int64.of_int Wal.header_size_bytes) (Int64.of_int Wal.frame_size_bytes)
     in
     ...
     let w2 =
       ...
       | Error e -> Alcotest.failf "reopen: %a" Wal.pp_error e
     in
     Alcotest.(check int) "trailing partial batch dropped" 1 (Wal.committed_frames w2);
     Alcotest.(check (option int)) "page 99 absent" None (Wal.find_page w2 99L);
     Lwt.return_unit)
;;
...
let test_recovery_drops_uncommitted_batch () =
  Lwt_main.run
    (let* d, w1 = fresh_wal () in
     ...
     Cstruct.BE.set_uint64 buf 8 0L;
     (* commit = false *)
     Cstruct.BE.set_uint64 buf 16 0L;
     (* bogus checksum *)
     let off =
       Int64.add (Int64.of_int Wal.header_size_bytes) (Int64.of_int Wal.frame_size_bytes)
     in
     ...
     let w2 =
       ...
       | Error e -> Alcotest.failf "reopen: %a" Wal.pp_error e
     in
     Alcotest.(check int) "uncommitted frame ignored" 1 (Wal.committed_frames w2);
...
let test_append_overwrites_after_reset () =
  Lwt_main.run
    (let* _, w = fresh_wal () in
     ...
     Alcotest.(check bool) "page 2 present" true (Wal.find_page w 2L <> None);
     Lwt.return_unit)
;;
...
let test_find_page_at_snapshot () =
  Lwt_main.run
    (let* _, w = fresh_wal () in
     ...
     Alcotest.(check (option int))
       "snap beyond committed sees latest"
       (Some 2)
       (Wal.find_page_at w 7L ~max_frame:100);
     Lwt.return_unit)
;;
...
let test_find_page_still_latest () =
  Lwt_main.run
    (let* _, w = fresh_wal () in
     ...
     Alcotest.(check (option int)) "latest frame is 2" (Some 2) (Wal.find_page w 3L);
     let* read =
  ...
  Alcotest.run
    ...
    ; ( "recovery"
      , [ Alcotest.test_case "replay committed" `Quick test_recovery_replays_committed
        ; Alcotest.test_case
            "drop partial trailing"
            `Quick
            test_recovery_drops_partial_trailing_batch
        ; Alcotest.test_case
            "drop uncommitted"
            `Quick
    ...
    ; ( "reset"
      , [ Alcotest.test_case "epoch starts, bumps, survives commit" `Quick test_epoch
        ...
        ; Alcotest.test_case
            ...
            `Quick
            test_append_overwrites_after_reset
        ] )
    ; ( "snapshot"
      , [ Alcotest.test_case "find_page_at bounds" `Quick test_find_page_at_snapshot
        ...
        ; Alcotest.test_case
            "find_page_at after recovery"
            `Quick
            test_find_page_at_after_recovery
        ] )
    ]
;;
