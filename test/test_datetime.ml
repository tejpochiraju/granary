...
let test_parse_date_only () =
  check_ok "date only" "2024-01-15" (parse "2024-01-15")
...
let test_parse_date_only_no_time () =
  ...
  | Ok dt ->
    Alcotest.(check string) "time defaults to 00:00:00" "00:00:00" (to_time dt)
  | Error e -> Alcotest.fail e
...
let test_parse_now_no_clock () =
  check_err "now without clock" (parse "now")

let test_parse_now_with_clock () =
  let fixed = 1705276800.0 in  (* 2024-01-15 00:00:00 UTC *)
...
let test_unixepoch_epoch () =
  ...
  | Error e -> Alcotest.fail e
  | Ok dt ->
    Alcotest.(check int64) "unixepoch 1970" 0L (to_unixepoch dt)
...
let test_unixepoch_known () =
  ...
  | Error e -> Alcotest.fail e
  | Ok dt ->
    Alcotest.(check int64) "unixepoch 2024-01-15" 1705276800L (to_unixepoch dt)

let test_to_datetime () =
  match parse "2024-06-15 09:05:03" with
  | Error e -> Alcotest.fail e
  | Ok dt ->
    Alcotest.(check string) "to_datetime" "2024-06-15 09:05:03" (to_datetime dt)

let test_datetime_roundtrip () =
  let cases = [
    "2024-01-01"; "2024-12-31"; "2000-02-29"; "1999-12-31";
  ] in
  List.iter (fun s ->
    match parse s with
...
let test_strftime_fractional_seconds () =
  ...
  | Ok dt ->
    ...
    Alcotest.(check bool) "%f starts with 03" true (String.length r >= 2 && String.sub r 0 2 = "03")
...
let test_pad4_large_year () =
  ...
  | Error e -> Alcotest.fail e
  | Ok dt ->
    Alcotest.(check string) "year 9999" "9999" (strftime "%Y" dt)
...
let test_pad3_values () =
  ...
  (match parse "2023-04-09" with  (* April 9 = day 99 *)
   | Error e -> Alcotest.fail e
   | Ok dt -> Alcotest.(check string) "%j day 99" "099" (strftime "%j" dt));
  (match parse "2023-04-10" with  (* April 10 = day 100 *)
   | Error e -> Alcotest.fail e
...
let () =
  Alcotest.run "datetime" [
    "parse", [
      ...
      Alcotest.test_case "invalid date separators" `Quick test_parse_invalid_date_bad_separators;
      Alcotest.test_case "invalid time in datetime" `Quick test_parse_invalid_time_in_datetime;
    ...
    "strftime", [
      ...
      Alcotest.test_case "fractional seconds %f"   `Quick test_strftime_fractional_seconds;
