...
let cleanup path =
  ...
  (try Unix.unlink (path ^ ".vacuum-tmp") with _ -> ());
  (try Unix.unlink (path ^ ".vacuum-tmp-wal") with _ -> ())
...
let row_text (row : Row.t) =
  String.concat "|" (Array.to_list (Array.map (function
    ...
    | Row.V_blob b -> "BLOB(" ^ string_of_int (Bytes.length b) ^ ")") row))
  ...
  run (
    ...
    let* () = Lwt_list.iter_s (fun i ->
      ...
      let sql = Printf.sprintf
        "INSERT INTO t (id, payload) VALUES (%d, '%s')" i payload in
      exec db sql) (List.init 500 Fun.id)
    in
    ...
    Alcotest.(check bool)
      (Printf.sprintf "vacuum shrank file (full=%d, after=%d)"
         size_full size_after)
      true (size_after < size_full);
    let* () = Db.close db in
    cleanup path;
    Lwt.return_unit)
...
let test_vacuum_preserves_tables_and_indexes () =
  run (
    ...
    Alcotest.(check string) "first row" "1|alice"
      (row_text (List.nth rows 0));
    Alcotest.(check string) "third row" "3|carol"
      (row_text (List.nth rows 2));
...
let () =
  Alcotest.run "vacuum" [
    "basic", [
      ...
      Alcotest.test_case "preserves indexes"       `Quick test_vacuum_preserves_tables_and_indexes;
      Alcotest.test_case "overflow blobs survive"  `Quick test_vacuum_with_overflow_blobs;
    ];
    "guards", [
