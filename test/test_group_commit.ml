  ...
  let rec loop i =
    ...
    then Lwt.return_unit
      ...
      let* () =
        exec_or_fail
          db
          (Printf.sprintf "INSERT INTO t (id, v) VALUES (%d, %d)" (base + i) (i * 7))
      in
      loop (i + 1)
  in
  loop 0
  ...
  run
    (let* db, path = open_wal_db () in
     Lwt.finalize
       (fun () ->
          let* () =
            exec_or_fail db "CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)"
          in
         ...
         sync count we're attributing to commit coalescing. *)
          ...
          Printf.printf
            "[#77] %d fibers x %d autocommits = %d commits in %.3fs ; %d fsyncs ; \
             commits/fsync = %.2f ; throughput = %.1f ops/s\n\
             %!"
            n_fibers
            per_fiber
            total_commits
         ...
         the run while still failing if coalescing is broken. *)
          let cps = float_of_int total_commits /. float_of_int (max 1 syncs_used) in
          Alcotest.(check bool)
            (Printf.sprintf "fsync coalescing >= 4 commits/fsync (got %.2f)" cps)
            true
            (cps >= 4.0);
          let* () = D.close db in
          Lwt.return_unit)
    ...
    (let* db, path = open_wal_db () in
     Lwt.finalize
       (fun () ->
          ...
          Printf.printf
            "[#77] single: %d ops / %.3fs = %.1f ops/s ; concurrent (%dx%d): %d ops / \
             %.3fs = %.1f ops/s ; ratio = %.2fx (informational)\n\
             %!"
            per_fiber
            dt_single
            single_ops_per_s
    ...
    (let* db, path = open_wal_db () in
     Lwt.finalize
       (fun () ->
          ...
          in
          Alcotest.(check int) "all rows present" (n_fibers * per_fiber) n;
          let rec check_fid fid =
            if fid = n_fibers
            then Lwt.return_unit
            else
              let* sr =
                D.query db (Printf.sprintf "SELECT COUNT(*) FROM t WHERE fid = %d" fid)
              in
              let s =
                match sr with
                | Ok s -> s
                | Error e -> Alcotest.failf "fid query: %a" D.pp_error e
              in
              let* rows = Lwt_stream.to_list s in
              let n =
                match rows with
              ...
              in
              Alcotest.(check int) (Printf.sprintf "fid=%d count" fid) per_fiber n;
              check_fid (fid + 1)
          in
          let* () = check_fid 0 in
          let* () = D.close db in
     ...
     Lwt.finalize
       (fun () ->
          ...
          let db =
            match db with
            | Ok d -> d
            | Error e -> Alcotest.failf "open: %a" D.pp_error e
          in
          let* () =
            exec_or_fail db "CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)"
          in
          ...
          let db2 =
            match db2 with
            | Ok d -> d
            | Error e -> Alcotest.failf "reopen: %a" D.pp_error e
          in
          let* sr = D.query db2 "SELECT COUNT(*) FROM t" in
          let s =
            match sr with
            | Ok s -> s
            | Error e -> Alcotest.failf "query after reopen: %a" D.pp_error e
          in
          let* rows = Lwt_stream.to_list s in
          let n =
            match rows with
            ...
            | _ -> Alcotest.failf "unexpected query result"
          in
          Alcotest.(check int) "rows durable after reopen" (n_fibers * per_fiber) n;
          let* () = D.close db2 in
          Lwt.return_unit)
       (fun () ->
          cleanup path;
...
let open_wal_with_injected_sync ~path ~fail_sync ~sync_calls =
  ...
  let wal_sync () =
    ...
    let* () = Lwt.pause () in
    if !fail_sync
    then Lwt.return_error "injected fsync failure"
    else (
      try
        Unix.fsync wal_fd;
        Lwt.return_ok ()
       ...
       (fun () ->
          ...
          let* outcome =
            Lwt.try_bind
              (fun () ->
                 let* tx = S.rw_begin st in
                 let* () = S.put tx 16 (Bytes.of_string "k") (Bytes.of_string "v") in
                 S.commit tx)
              (fun () -> Lwt.return `Ok)
              (fun _ -> Lwt.return `Err)
          in
          Alcotest.(check bool)
            "lone writer observes the injected failure"
            true
            (outcome = `Err);
          let* () = S.close st in
          Lwt.return_unit)
...
let () =
  Alcotest.run
    ...
    [ ( "correctness"
      , [ Alcotest.test_case
            "8 fibers x 50 autocommits all present"
            `Quick
            test_group_commit_correctness
        ; Alcotest.test_case
            "8 fibers durable after reopen"
            `Quick
            test_group_commit_durable_after_reopen
        ] )
    ; ( "batching"
      , [ Alcotest.test_case
            "8 fibers coalesce >=4 commits per fsync"
            `Quick
            test_group_commit_batching
        ] )
    ; ( "fault_injection"
      , [ Alcotest.test_case
            "fsync failure propagates to drainer + joiners"
            `Quick
            test_inject_fsync_failure_propagates_to_all_writers
        ; Alcotest.test_case
            "fsync failure observed by lone writer"
            `Quick
            test_inject_fsync_failure_lone_writer
        ] )
    ; ( "throughput"
      , [ Alcotest.test_case
            "8 fibers vs single-fiber (informational)"
            `Slow
            test_group_commit_throughput
        ] )
    ]
;;
