...
let make_dev () =
  ...
  let resize ~n_pages = ensure (Int64.to_int n_pages); Lwt.return_ok () in
  (read_page, write_page, sync, resize, reads)

let open_store () =
  let (read_page, write_page, sync, resize, reads) = make_dev () in
  let* r =
    S.open_block ~read_page ~write_page ~sync ~resize
      ~n_pages:0L ~close:(fun () -> Lwt.return_unit)
  in
  match r with
...
let test_pinned_snapshot_no_growing_tail () =
  ...
  run begin
    ...
    let rec cycle c =
      if c >= cycles then Lwt.return_unit
      else begin
        (* Writer churn into the same tree. *)
        let rec churn k =
          if k >= commits_per_cycle then Lwt.return_unit
          else
...
let () =
  Alcotest.run "pager_snapshot_pin" [
    "pin", [
      Alcotest.test_case "held snapshot has no growing re-fetch tail"
        `Quick test_pinned_snapshot_no_growing_tail;
    ];
  ]
