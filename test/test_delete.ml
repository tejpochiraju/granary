...
let qcheck_delete_by_id =
  QCheck.Test.make
    ...
    ~count:10_000
    QCheck.(pair (list_size Gen.(1 -- 20) (int_range 1 1000)) (int_range 1 1000))
    (fun (ids, k) ->
       (* Deduplicate ids to avoid duplicate primary-key-like entries. *)
       let unique_ids = List.sort_uniq compare ids in
       if unique_ids = []
       then true
       else (
            ...
            | Ok stream ->
              ...
              let remaining =
                List.map
                  ...
                  rows
              in
              let expected = List.filter (fun id -> id <> k) unique_ids in
              (* deleted row absent *)
    ...
    (fun ns ->
       ...
       Lwt_main.run
         (let* _ = Db.execute db "CREATE TABLE t (n INTEGER)" in
          ...
          match result with
          | Error _ -> Lwt.return false
          | Ok got_count -> Lwt.return (got_count = List.length ns)))
;;
    ...
    ; ( "sema_errors"
      , [ Alcotest.test_case "unknown_table" `Quick delete_unknown_table
        ; Alcotest.test_case
            ...
            `Quick
            delete_unknown_column_in_where
        ] )
    ; ( "runtime_routing"
      , [ Alcotest.test_case
            "delete_via_query_is_runtime_error"
            `Quick
            delete_via_query_returns_runtime_error
        ] )
    ; ( "indexes"
      , [ Alcotest.test_case
            "delete_removes_index_entries"
            `Quick
            delete_removes_index_entries
        ; Alcotest.test_case
            "delete_then_reinsert_unique"
            `Quick
            delete_then_reinsert_unique
        ; Alcotest.test_case
            "delete_removes_all_index_entries"
            `Quick
            delete_removes_all_index_entries
        ] )
    ; ( "row_count"
      , [ Alcotest.test_case "delete_returns_count" `Quick delete_returns_count
        ; Alcotest.test_case
            "delete_returns_zero_when_no_match"
            `Quick
            delete_returns_zero_when_no_match
        ; Alcotest.test_case
            "delete_all_returns_total_count"
            `Quick
            delete_all_returns_total_count
        ] )
    ; ( "qcheck"
      , List.map
          ...
          [ qcheck_delete_by_id; qcheck_delete_all_count_matches ] )
    ]
;;
