...
let bench_inserts_bulk ~label db =
  let dt = time_it (label ^ ": bulk INSERT in 1 txn") (fun () ->
    exec db "BEGIN";
    for i = 0 to n - 1 do
      exec db (Printf.sprintf "INSERT INTO t (id, name, val) VALUES (%d, 'row-%d', %d)" i i (i * 3))
    done;
    exec db "COMMIT")
  in
...
let bench_inserts_autocommit ~label db =
  let dt = time_it (label ^ ": INSERT autocommit") (fun () ->
    for i = n to (2 * n) - 1 do
      exec db (Printf.sprintf "INSERT INTO t (id, name, val) VALUES (%d, 'row-%d', %d)" i i (i * 3))
    done)
  in
...
let bench_point_select ~label db =
  let dt = time_it (label ^ ": point SELECT by PK") (fun () ->
    for i = 0 to n - 1 do
...
let bench_full_scan ~label db =
  let dt = time_it (label ^ ": full SELECT scan ×10") (fun () ->
    for _ = 0 to 9 do
...
let bench_updates ~label db =
  let dt = time_it (label ^ ": UPDATE by PK") (fun () ->
    exec db "BEGIN";
    for i = 0 to n - 1 do
...
let bench_deletes ~label db =
  let dt = time_it (label ^ ": DELETE by PK") (fun () ->
    exec db "BEGIN";
    for i = 0 to n - 1 do
...
let test_bench_sqlite3_optional () =
  ...
  (match sqlite3 with
   ...
   | Some bin ->
     ...
     for i = 0 to n - 1 do
       Printf.fprintf oc "INSERT INTO t (id, name, val) VALUES (%d, 'row-%d', %d);\n" i i (i * 3)
     done;
     output_string oc "COMMIT;\n";
     close_out oc;
     let t0 = Unix.gettimeofday () in
     let exit_code = Sys.command (Printf.sprintf "%s %s < %s > /dev/null" bin path script_path) in
     let dt = Unix.gettimeofday () -. t0 in
     if exit_code = 0 then begin
       Printf.eprintf "BENCH: %-50s %8.4f s\n%!" "sqlite3 CLI: bulk INSERT in 1 txn" dt;
       throughput "sqlite3 CLI: INSERT" n dt
     end else
       Printf.eprintf "BENCH: sqlite3 subprocess exited %d (skipping)\n%!" exit_code;
     (try Unix.unlink script_path with _ -> ()));
  (try Unix.unlink path with _ -> ())

let () =
  Alcotest.run "perf_bench" [
    "bench", [
      ...
      Alcotest.test_case "sqlite3 CLI baseline (optional)" `Slow test_bench_sqlite3_optional;
    ];
  ]
