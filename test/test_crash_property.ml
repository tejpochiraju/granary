...
let prop_snapshot_iso =
  QCheck.Test.make ~count:50 ~name:"snapshot iso: RO never sees later writes"
    QCheck.(list_small (pair (string_size (Gen.return 6))
                             (string_size (Gen.return 8))))
    (fun kvs ->
      ...
      try
        run (
          ...
          let* () = Lwt_list.iter_s (fun (k, v) ->
            S.put tx 16 (bs k) (bs v)) kvs
          in
          let* () = S.commit tx in
          (* Every key must still be invisible through the old snapshot. *)
          let ok = ref true in
          let* () = Lwt_list.iter_s (fun (k, _) ->
          ...
          let* () = S.close st in
          Lwt.return !ok)
      with _ -> cleanup path; false
      |> fun result -> cleanup path; result)
...
let read_all_keys st keys =
  ...
  let* observed = Lwt_list.map_s (fun k ->
...
let matches_any_prefix observed expected_prefixes =
  ...
  matches empty
  || List.exists matches expected_prefixes

let prop_crash_then_open =
  QCheck.Test.make ~count:30 ~name:"crash recovery: WAL truncation → prefix"
    QCheck.(triple
              (list_small (list_small
                            (pair (string_size (Gen.return 4))
                                  (string_size (Gen.return 6)))))
              (int_range 0 4096)
              (int_range 0 4))
    (fun (commits, truncate_offset_extra, _seed) ->
      ...
      try
        ...
        let result = run (
          ...
          let wal_size =
            try Unix.((stat (path ^ "-wal")).st_size) with _ -> 0
          in
          let keep =
            if wal_size = 0 then 0
            else (truncate_offset_extra mod (wal_size + 1))
          in
...
let () =
  Alcotest.run "crash_property" [
    "qcheck", List.map QCheck_alcotest.to_alcotest [
      prop_snapshot_iso;
      prop_crash_then_open;
    ];
  ]
