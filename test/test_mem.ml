...
let read_zeros_test () =
  Lwt_main.run
    (let m = Mem.create ~n_pages:4L () in
     ...
     match r with
     | Error e -> Alcotest.failf "unexpected error: %a" Mem.pp_error e
     | Ok () ->
...
let write_last_valid_page_test () =
  Lwt_main.run
    (let m = Mem.create ~n_pages:4L () in
     ...
     (match rr with
      | Error e -> Alcotest.failf "read error: %a" Mem.pp_error e
      | Ok () -> Alcotest.(check char) "byte 0 = Z" 'Z' (Cstruct.get_char rbuf 0));
     Lwt.return_unit)
;;
...
let write_page_0_test () =
  Lwt_main.run
    (let m = Mem.create ~n_pages:1L () in
     ...
     (match rr with
      | Error e -> Alcotest.failf "read error: %a" Mem.pp_error e
      | Ok () -> Alcotest.(check char) "byte 0 = X" 'X' (Cstruct.get_char rbuf 0));
     Lwt.return_unit)
;;
...
let resize_to_same_size_test () =
  Lwt_main.run
    (let m = Mem.create ~n_pages:4L () in
     ...
     (match rr with
      | Error e -> Alcotest.failf "read error: %a" Mem.pp_error e
      | Ok () -> Alcotest.(check char) "data preserved" 'Q' (Cstruct.get_char rbuf 0));
     Lwt.return_unit)
;;
    ...
    (fun n ->
       ...
       Lwt_main.run
         (let* r = Mem.resize m ~n_pages:(Int64.of_int n) in
          match r with
          | Error _ -> Lwt.return false
          | Ok () -> Lwt.return (Mem.n_pages m = Int64.of_int n)))
;;
    ...
    (fun (size, page_id) ->
       ...
       Lwt_main.run
         (let buf = make_buf () in
          ...
          match rr with
          | Error _ -> Lwt.return false
          | Ok () -> Lwt.return (Cstruct.for_all (fun c -> c = '\x00') buf)))
;;
  ...
  Alcotest.run
    ...
    ; ( "resize"
      , [ Alcotest.test_case "resize_grow" `Quick resize_grow_test
        ...
        ; Alcotest.test_case
            "resize_grow_preserves_data"
            `Quick
            resize_grow_preserves_data_test
    ...
    ; "pp_error", [ Alcotest.test_case "pp_error_format" `Quick pp_error_format_test ]
    ; "qcheck", qcheck_tests
    ]
;;
