...
let tokenize s =
  ...
  let rec loop acc =
    ...
    let acc' = t :: acc in
    if t = Parser.EOF then List.rev acc' else loop acc'
  in
  loop []
;;
...
let lit_tests =
  ...
  ; check
      "int_neg_big"
      [ Parser.MINUS; Parser.INT_LIT 9999999999L; Parser.EOF ]
      "-9999999999"
...
let ws_tests =
  ...
  ; check
      "comment_inline"
      [ Parser.CREATE; Parser.TABLE; Parser.EOF ]
      "CREATE -- comment\nTABLE"
  ]
;;
