...
let test_snapshot_reads_isolated_from_newer_frames () =
  Lwt_main.run begin
    ...
    (match fr with Ok () -> () | Error _ -> Alcotest.fail "flush");
...
let test_main_cache_invalidated_after_clear_dirty () =
  ...
  Lwt_main.run begin
    ...
    Alcotest.(check int) "clear_dirty without dirty keeps cache"
      0x30 (read_byte r3);
    ...
    Alcotest.(check int) "clear_dirty drops main-key cache for dirtied page"
      0xCC (read_byte r4);
    Lwt.return_unit
  end
  ...
  Alcotest.run "pager_versioned_cache" [
    "snapshot", [
      Alcotest.test_case "older reader isolated from newer WAL frames"
        `Quick test_snapshot_reads_isolated_from_newer_frames;
      Alcotest.test_case "writer dirty invisible to concurrent reader"
        `Quick test_writer_dirty_invisible_to_concurrent_readers;
      Alcotest.test_case "main cache invalidated after clear_dirty"
        `Quick test_main_cache_invalidated_after_clear_dirty;
    ];
  ]
