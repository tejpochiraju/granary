...
let test_tokenizer_unicode_german () =
  ...
  Alcotest.(check string) "folded term" "grösse" (List.hd toks).Tok.term
;;
...
let test_tokenizer_unicode_turkish_dotted_i () =
  ...
  Alcotest.(check string)
    "Turkish dotted-I fold"
    "i\xCC\x87stanbul"
    (List.hd toks).Tok.term
;;
...
let test_fts_insert () =
  run (fun () ->
    ...
    let* r1 =
      D.execute
        db
        "INSERT INTO docs (title, body) VALUES ('Hello World', 'The quick brown fox')"
    in
    let* r2 =
      D.execute
        db
        "INSERT INTO docs (title, body) VALUES ('OCaml intro', 'Functional programming')"
    in
    match r1, r2 with
...
let test_match_basic () =
  run (fun () ->
    ...
    let* _ =
      D.execute
        db
        "INSERT INTO docs (title, body) VALUES ('OCaml intro', 'Functional language')"
    in
    let* _ =
      D.execute
        db
        "INSERT INTO docs (title, body) VALUES ('Python guide', 'Dynamic language')"
    in
    let* _ =
      D.execute
        db
        "INSERT INTO docs (title, body) VALUES ('OCaml advanced', 'Type systems')"
    in
...
let test_rank_column () =
  run (fun () ->
    ...
    let* _ =
      D.execute db "INSERT INTO docs (body) VALUES ('ocaml ocaml ocaml tutorial')"
    in
...
let test_bm25_per_term_tf () =
  run (fun () ->
    ...
    let* _ =
      D.execute db "INSERT INTO docs (body) VALUES ('ocaml ocaml ocaml ocaml tutorial')"
    in
  ...
  Alcotest.run
    ...
    [ ( "ddl"
      , [ Alcotest.test_case "create_fts_table" `Quick test_create_fts_table
        ; Alcotest.test_case
            "create_fts_duplicate_fails"
            `Quick
            test_create_fts_duplicate_fails
        ; Alcotest.test_case "create_fts_single_col" `Quick test_create_fts_single_col
        ; Alcotest.test_case
            "fts_persists_across_reopen"
            `Quick
            test_fts_persists_across_reopen
        ] )
    ; ( "tokenizer"
      , [ Alcotest.test_case "basic" `Quick test_tokenizer_basic
        ...
        ; Alcotest.test_case
            "unicode_french_case"
            `Quick
            test_tokenizer_unicode_french_case
        ; Alcotest.test_case
            "unicode_turkish_i"
            `Quick
            test_tokenizer_unicode_turkish_dotted_i
    ...
    ; ( "match"
      , [ Alcotest.test_case "basic" `Quick test_match_basic
        ...
        ; Alcotest.test_case
            "phrase_non_adjacent"
            `Quick
            test_phrase_non_adjacent_no_match
        ] )
    ; ( "rank"
