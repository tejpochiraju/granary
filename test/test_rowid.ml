...
let check_bytes_equal label expected actual =
  if not (Bytes.equal expected actual) then
    Alcotest.failf "%s: expected %s got %s" label
      (Bytes.to_seq expected |> Seq.map (fun c -> Printf.sprintf "%02x" (Char.code c)) |> List.of_seq |> String.concat "")
      (Bytes.to_seq actual   |> Seq.map (fun c -> Printf.sprintf "%02x" (Char.code c)) |> List.of_seq |> String.concat "")
...
let test_order_preserving () =
  List.iter (fun (a, b) ->
    let ea = encode a and eb = encode b in
    let cmp = Bytes.compare ea eb in
    if not (cmp < 0) then
      Alcotest.failf "order violated: encode(%Ld)=%s should be < encode(%Ld)=%s"
        a (hex_of_bytes ea) b (hex_of_bytes eb)
  ) order_pairs

let test_equal_encodes_equal () =
  List.iter (fun v ->
    let ea = encode v and eb = encode v in
    if Bytes.compare ea eb <> 0 then
      Alcotest.failf "encode(%Ld) not equal to itself" v
...
let roundtrip_values = [
  0L; 1L; -1L; 42L; -42L; 127L; -127L; 128L; -128L;
  Int64.max_int; Int64.min_int;
  Int64.sub Int64.max_int 1L; Int64.add Int64.min_int 1L;
  1000000L; -1000000L;
]
...
let test_roundtrip () =
  List.iter (fun v ->
    ...
    if not (Int64.equal v v') then
      Alcotest.failf "roundtrip failed: %Ld -> %Ld" v v'
  ) roundtrip_values
...
let test_encoded_length () =
  List.iter (fun v ->
    ...
    if len <> 8 then
      Alcotest.failf "encode(%Ld) length=%d, expected 8" v len
  ) roundtrip_values
...
let test_sort_agrees () =
  ...
  if not (List.equal Int64.equal sorted_int64 decoded) then begin
    let fmt l = List.map (Printf.sprintf "%Ld") l |> String.concat ", " in
    Alcotest.failf "sort mismatch:\n  by int64: [%s]\n  by bytes: [%s]"
      (fmt sorted_int64) (fmt decoded)
  end
...
let prop_roundtrip =
  QCheck.Test.make ~count:10000 ~name:"rowid roundtrip"
    QCheck.int64 (fun i ->
      Int64.equal i (decode (encode i)))

let prop_8_bytes =
  QCheck.Test.make ~count:10000 ~name:"always 8 bytes"
    QCheck.int64 (fun i ->
      Bytes.length (encode i) = 8)

let prop_order_preserving =
  QCheck.Test.make ~count:10000 ~name:"order preserving"
...
let prop_injective =
  QCheck.Test.make ~count:10000 ~name:"injective"
    (QCheck.pair QCheck.int64 QCheck.int64) (fun (a, b) ->
      if Int64.equal a b then true
      else not (Bytes.equal (encode a) (encode b)))
...
let () =
  let alcotest_tests = [
    "order_preserving",   [ Alcotest.test_case "fixed pairs"        `Quick test_order_preserving     ];
    "equal_encodes",      [ Alcotest.test_case "encode = encode"     `Quick test_equal_encodes_equal  ];
    ...
    "boundary_min_int",   [ Alcotest.test_case "min_int -> 0x00..00" `Quick test_boundary_min_int     ];
    ...
    "boundary_max_int",   [ Alcotest.test_case "max_int -> 0xff..ff" `Quick test_boundary_max_int     ];
