...
let cases_int64 = [
  0L; 1L; -1L; 127L; -127L; 128L; -128L;
  16383L; -16383L; 16384L; -16384L;
...
let prop_uint64_roundtrip =
  QCheck.Test.make ~count:10000 ~name:"uint64 roundtrip"
    QCheck.int64 (fun i ->
...
let prop_int64_roundtrip =
  QCheck.Test.make ~count:10000 ~name:"int64 roundtrip"
    QCheck.int64 (fun i ->
      let buf = Buffer.create 16 in
...
let prop_max_bytes =
  QCheck.Test.make ~count:10000 ~name:"max 10 bytes"
    QCheck.int64 (fun i ->
      let buf = Buffer.create 16 in
      Varint.encode_int64 buf i;
...
let prop_deterministic =
  QCheck.Test.make ~count:10000 ~name:"deterministic"
    QCheck.int64 (fun i ->
      let b1 = Buffer.create 8 in Varint.encode_int64 b1 i;
      let b2 = Buffer.create 8 in Varint.encode_int64 b2 i;
...
let () =
  let uint64_rt_tests =
    List.map (fun v ->
      Alcotest.test_case
        (Printf.sprintf "roundtrip uint64 %Ld" v) `Quick (roundtrip_uint64 v))
      cases_uint64
  in
  let int64_rt_tests =
    List.map (fun v ->
      Alcotest.test_case
        (Printf.sprintf "roundtrip int64 %Ld" v) `Quick (roundtrip_int64 v))
      cases_int64
  in
  let uint64_len_tests =
    List.map (fun (v, expected, label) ->
  ...
  let qcheck_tests =
    List.map QCheck_alcotest.to_alcotest
      [ prop_uint64_roundtrip
      ; prop_int64_roundtrip
      ; prop_max_bytes
      ; prop_deterministic ]
  in
  Alcotest.run "varint" [
