...
let is_null = function Row.V_null -> true | _ -> false
...
let avg_returns_real () =
  ...
  match r.(0) with
  | Row.V_real f ->
    Alcotest.(check (float 1e-9)) "avg = 1.5" 1.5 f
...
let multiple_aggregates () =
  ...
  let rows = query_ok db
    "SELECT COUNT(*), SUM(n), AVG(n), MIN(n), MAX(n) FROM t" in
  let r = List.hd rows in
...
let group_by_basic () =
  ...
  let by_label =
    List.map (fun r -> (text_of_value r.(0), int_of_value r.(1))) rows
    |> List.sort compare
  in
...
let group_by_null_groups () =
  ...
  let null_count =
    List.fold_left (fun acc r ->
      if is_null r.(0) then int_of_value r.(1) else acc
    ) (-1) rows
  in
  let a_count =
    List.fold_left (fun acc r ->
...
let having_basic () =
  ...
  let rows = query_ok db
    "SELECT s, COUNT(*) FROM t GROUP BY s HAVING COUNT(*) > 1" in
...
let having_filters_all () =
  ...
  exec db "INSERT INTO t (s, n) VALUES ('b', 2)";
  let rows = query_ok db
    "SELECT s, COUNT(*) FROM t GROUP BY s HAVING COUNT(*) > 10" in
...
let group_by_with_order_by () =
  ...
  exec db "INSERT INTO t (s, n) VALUES ('a', 4)";
  let rows = query_ok db
    "SELECT s, COUNT(*) FROM t GROUP BY s ORDER BY s ASC" in
...
let setup_int_table values =
  ...
  exec db "CREATE TABLE t (n INTEGER)";
  List.iter (fun v ->
    exec db (Printf.sprintf "INSERT INTO t (n) VALUES (%d)" v)
  ) values;
  db
    ...
    (fun pairs ->
       ...
       List.iter (fun (k, v) ->
         exec db (Printf.sprintf
                    "INSERT INTO t (s, n) VALUES ('g%d', %d)" k v)
       ) pairs;
       let rows = query_ok db "SELECT s, COUNT(*) FROM t GROUP BY s" in
       (* Total count across all groups must equal total rows. *)
       let total = List.fold_left (fun acc r ->
         match r.(1) with
       ...
       List.length rows = expected_groups)
