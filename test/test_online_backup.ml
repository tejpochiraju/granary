...
let test_copy_physical_equality () =
  ...
  @@ Lwt.finalize
       (fun () ->
          ...
          let* cr = UnixStore.copy_to_file src ~dest:dst_path in
          (match cr with
           | Error e -> Alcotest.failf "copy: %a" S.pp_error e
           | Ok () -> ());
          let* () = S.close src in
          let read_file path =
          ...
          Alcotest.(check int)
            "dest file size"
            src_stat.Unix.st_size
            dst_stat.Unix.st_size;
          Lwt.return_unit)
       (fun () ->
          cleanup src_path;
          cleanup dst_path;
...
let test_copy_multi_page_db () =
  ...
  @@ Lwt.finalize
       (fun () ->
          ...
          let* cr = UnixStore.copy_to_file src ~dest:dst_path in
          (match cr with
           | Error e -> Alcotest.failf "copy multi: %a" S.pp_error e
           | Ok () -> ());
          let* () = S.close src in
          let* dst_r = UnixStore.open_file ~path:dst_path () in
          let dst = ok_store dst_r in
          let* () =
            S.with_ro dst (fun tx ->
              ...
              Alcotest.check
                bytes_opt_eq
                "key0000"
                (Some (bs (String.make 200 'X' ^ "0000")))
                v;
              let* v199 = S.get tx 16 (bs "key0199") in
              Alcotest.check
                bytes_opt_eq
                "key0199"
                (Some (bs (String.make 200 'X' ^ "0199")))
                v199;
              Lwt.return_unit)
          in
          let* () = S.close dst in
  ...
  @@ Lwt.finalize
       (fun () ->
          ...
          let sink : S.page_sink =
            fun ~page_id:_ ~page:_ ->
            incr count;
            if !count >= 2 then Lwt.fail (Failure "sink error test") else Lwt.return_unit
          in
          let* result =
            Lwt.catch
              (fun () ->
          ...
          (match result with
           ...
           | Error msg ->
             Alcotest.(check bool) "sink error message" true (String.length msg > 0));
          let* () = S.close src in
          Lwt.return_unit)
       (fun () ->
          cleanup src_path;
...
let test_copy_over_existing_dest () =
  ...
  @@ Lwt.finalize
       (fun () ->
          ...
          (match cr1 with
           | Error e -> Alcotest.failf "copy1: %a" S.pp_error e
           | Ok () -> ());
          let* () = S.close src1 in
          cleanup src_path1;
          ...
          (match cr2 with
           | Error e -> Alcotest.failf "copy2: %a" S.pp_error e
           | Ok () -> ());
          let* () = S.close src2 in
          cleanup src_path2;
          let* dst_r = UnixStore.open_file ~path:dst_path () in
