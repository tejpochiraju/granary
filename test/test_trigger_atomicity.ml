  ...
  run (
    ...
    let* () = exec db
      "CREATE TRIGGER t_ai AFTER INSERT ON t \
       BEGIN INSERT INTO no_such_table VALUES (NEW.id); END" in
...
let test_after_update_failure_rolls_back () =
  run (
    ...
    let* () = exec db
      "CREATE TRIGGER t_au AFTER UPDATE ON t \
       BEGIN INSERT INTO no_such_table VALUES (NEW.val); END" in
  ...
  run (
    ...
    let* () = exec db
      "CREATE TRIGGER t_ad AFTER DELETE ON t \
       BEGIN INSERT INTO no_such_table VALUES (OLD.val); END" in
    ...
    let* () = exec db
      "CREATE TRIGGER t_ai AFTER INSERT ON t \
       BEGIN \
         ...
         INSERT INTO no_such_table VALUES (NEW.id); \
       END" in
...
let test_before_delete_fires_on_replace () =
  run (
    let* db = open_mem () in
    let* () = exec db
      "CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT)" in
    ...
    let* () = exec db
      "CREATE TRIGGER t_bd BEFORE DELETE ON t \
       BEGIN INSERT INTO del_log VALUES (OLD.id, OLD.name); END" in
    let* () = exec db "INSERT INTO t VALUES (1, 'alice')" in
    (* Same UNIQUE name → conflict → REPLACE displaces row id=1. *)
    let* () = exec db
      "INSERT OR REPLACE INTO t (id, name) VALUES (2, 'alice')" in
    ...
    (match rows with
     | [r] when Array.length r >= 2 ->
       Alcotest.(check (option int)) "displaced id"
         (Some 1) (int_of_value r.(0));
       (match r.(1) with
...
let test_before_delete_failure_rolls_back_replace () =
  run (
    ...
    let* () = exec db
      "CREATE TRIGGER t_bd BEFORE DELETE ON t \
       BEGIN INSERT INTO no_such_table VALUES (OLD.id); END" in
    let* () = exec db "INSERT INTO t VALUES (1, 'alice')" in
    let* r = Db.execute db
      "INSERT OR REPLACE INTO t (id, name) VALUES (2, 'alice')" in
...
let test_before_update_fires_on_upsert () =
  run (
    let* db = open_mem () in
    let* () = exec db
      "CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER)" in
    let* () = exec db
      "CREATE TABLE upd_log (rid INTEGER, old_val INTEGER, new_val INTEGER)" in
    let* () = exec db
      "CREATE TRIGGER t_bu BEFORE UPDATE ON t \
       BEGIN INSERT INTO upd_log VALUES (OLD.id, OLD.val, NEW.val); END" in
    let* () = exec db "INSERT INTO t VALUES (1, 100)" in
    let* () = exec db
      "INSERT INTO t (id, val) VALUES (1, 999) \
       ON CONFLICT(id) DO UPDATE SET val = 200" in
...
let test_nested_after_trigger_no_deadlock () =
  run (
    ...
    let* () = exec db
      "CREATE TRIGGER t_ai AFTER INSERT ON t \
       BEGIN INSERT INTO mid VALUES (NEW.id); END" in
    let* () = exec db
      "CREATE TRIGGER mid_ai AFTER INSERT ON mid \
       BEGIN INSERT INTO deep VALUES (NEW.tid); END" in
...
(* ------------------------------------------------------------------ *)
let () =
  Alcotest.run "trigger_atomicity" [
    "after_atomicity", [
      Alcotest.test_case "AFTER INSERT failure rolls back DML" `Quick
        test_after_insert_failure_rolls_back;
      Alcotest.test_case "AFTER UPDATE failure rolls back DML" `Quick
        test_after_update_failure_rolls_back;
      Alcotest.test_case "AFTER DELETE failure rolls back DML" `Quick
        test_after_delete_failure_rolls_back;
      Alcotest.test_case "AFTER trigger writes share parent txn" `Quick
        test_after_insert_writes_in_same_txn;
      Alcotest.test_case "AFTER multi-stmt rollback is atomic" `Quick
        test_after_trigger_multi_stmt_atomicity;
    ];
    "before_displaced", [
      Alcotest.test_case "BEFORE DELETE fires on REPLACE-displaced row" `Quick
        test_before_delete_fires_on_replace;
      Alcotest.test_case "BEFORE DELETE failure rolls back REPLACE" `Quick
        test_before_delete_failure_rolls_back_replace;
      Alcotest.test_case "BEFORE UPDATE fires on UPSERT-displaced row" `Quick
        test_before_update_fires_on_upsert;
      Alcotest.test_case "BEFORE UPDATE failure rolls back UPSERT" `Quick
        test_before_update_failure_rolls_back_upsert;
    ];
    "nested", [
      Alcotest.test_case "nested AFTER triggers (no deadlock)" `Quick
