...
let check_rows expected actual =
  ...
  List.iteri
    (fun i (exp, got) -> Alcotest.check row_testable (Printf.sprintf "row %d" i) exp got)
...
let tests =
  [ { name = "create_insert_select_all"
    ...
    ; query = "SELECT * FROM t"
    ; expected =
        [ [| Db.V_int 1L; Db.V_text "hello" |]; [| Db.V_int 2L; Db.V_text "world" |] ]
    }
  ; { name = "select_where_equality"
    ...
    ; expected = [ [| Db.V_int 10L; Db.V_text "alice" |] ]
    }
  ; { name = "order_by_asc"
    ; setup =
    ...
    ; expected = [ [| Db.V_int 1L |]; [| Db.V_int 2L |]; [| Db.V_int 3L |] ]
    }
  ; { name = "order_by_desc"
    ; setup =
        [ "CREATE TABLE t (n INTEGER)"
    ...
    ; expected = [ [| Db.V_int 3L |]; [| Db.V_int 2L |]; [| Db.V_int 1L |] ]
    }
  ; { name = "limit_basic"
    ; setup =
        [ "CREATE TABLE t (n INTEGER)"
    ...
    ; expected = [ [| Db.V_int 10L |]; [| Db.V_int 20L |]; [| Db.V_int 30L |] ]
    }
  ; { name = "limit_offset"
    ; setup =
        [ "CREATE TABLE t (n INTEGER)"
    ...
    ; expected = [ [| Db.V_int 30L |]; [| Db.V_int 40L |] ]
    }
  ; { name = "real_values"
    ; setup =
    ...
    ; expected = [ [| Db.V_real (-1.5) |]; [| Db.V_real 3.14 |] ]
    }
  ; { name = "null_values"
    ; setup =
        [ "CREATE TABLE t (n INTEGER)"
  ...
  ; { name = "null_sorts_first_asc"
    ...
    ; query = "SELECT * FROM t ORDER BY n ASC"
    ; expected = [ [| Db.V_null |]; [| Db.V_int 1L |]; [| Db.V_int 2L |] ]
    }
  ; { name = "index_lookup"
    ; setup =
        [ "CREATE TABLE t (id INTEGER, name TEXT)"
    ...
    ; expected = [ [| Db.V_int 1L; Db.V_text "alice" |] ]
    }
  ; { name = "empty_table"
  ...
  ; { name = "left_join_with_nulls"
    ...
    ; expected = [ [| Db.V_int 1L; Db.V_text "one" |]; [| Db.V_int 2L; Db.V_null |] ]
    }
  ; { name = "three_way_join"
    ; setup =
  ...
  ; { name = "join_with_where_and_order"
    ...
    ; query =
        "SELECT x.id, x.n, y.m FROM x JOIN y ON x.id=y.id WHERE x.n >= 20 ORDER BY x.n \
         DESC"
    ; expected =
  ...
  ; { name = "group_by_single"
    ; setup =
        [ "CREATE TABLE t (cat TEXT, n INTEGER)"
        ; "INSERT INTO t (cat, n) VALUES ('a', 1)"
        ; "INSERT INTO t (cat, n) VALUES ('a', 2)"
        ; "INSERT INTO t (cat, n) VALUES ('b', 5)"
        ]
  ...
  ; { name = "union_all"
    ...
    ; expected =
        [ [| Db.V_int 1L |]; [| Db.V_int 1L |]; [| Db.V_int 2L |]; [| Db.V_int 2L |] ]
    }
  ; { name = "union_dedup"
    ; setup =
        [ "CREATE TABLE t (n INTEGER)"
  ...
  ; (* ----- FK + trigger tests (6) ----- *)
    ...
    ; setup =
        ...
        ; "CREATE TABLE child (id INTEGER, pid INTEGER REFERENCES parent(id) ON DELETE \
           CASCADE)"
  ...
  ; { name = "trigger_after_insert_log"
    ; setup =
        ...
        ; "CREATE TRIGGER trg AFTER INSERT ON t BEGIN INSERT INTO log (msg) VALUES \
           ('inserted'); END"
        ; "INSERT INTO t (n) VALUES (1)"
        ; "INSERT INTO t (n) VALUES (2)"
        ]
  ...
  ; { name = "trigger_before_insert_alter_row"
    ; setup =
        ...
        ; "CREATE TRIGGER trg BEFORE INSERT ON t FOR EACH ROW BEGIN UPDATE t SET doubled \
           = NEW.n * 2 WHERE n = NEW.n; END"
  ...
  ; { name = "trigger_update_audit"
    ; setup =
        ...
        ; "CREATE TRIGGER trg AFTER UPDATE ON t BEGIN INSERT INTO audit (id, oldv, newv) \
           VALUES (OLD.id, OLD.v, NEW.v); END"
  ...
  ; { name = "coalesce_picks_first_non_null"
    ; setup =
        ...
        ; "INSERT INTO t (a, b) VALUES (NULL, 'fallback')"
        ]
  ...
  ; { name = "sqlite_master_lists_tables"
    ; setup = [ "CREATE TABLE foo (a INTEGER)"; "CREATE TABLE bar (b TEXT)" ]
    ; query = "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name"
    ; expected = [ [| Db.V_text "bar" |]; [| Db.V_text "foo" |] ]
    }
  ; { name = "sqlite_master_index_entry"
    ; setup = [ "CREATE TABLE t (a INTEGER)"; "CREATE INDEX my_idx ON t (a)" ]
...
let tests_for_btree =
  List.filter
    (fun t ->
       ...
       && not (List.mem t.name btree_known_failures))
    tests
;;
       ...
       Lwt.finalize
         (fun () ->
            ...
            (match result with
             ...
             | Ok () ->
               Alcotest.failf
                 "%s: expected Runtime error for UNIQUE violation, got Ok"
                 backend_name
             | Error e ->
               Alcotest.failf
                 "%s: expected Runtime error, got %s"
                 backend_name
                 (error_msg e));
            Lwt.return_unit)
         (fun () -> close_db db)))
...
let () =
  Alcotest.run
    "conformance"
    [ "mem", mem_suite (); "unix_file", unix_file_suite (); "mirage", mirage_suite () ]
;;
