...
let test_torn_write_midframe () =
  run (
    ...
    Lwt.finalize (fun () ->
      ...
      let extra = 1234 in
      let fd =
        Unix.openfile (path ^ "-wal") [Unix.O_WRONLY] 0o644
      in
      ...
      let* () = S.ro_end tx in
      Alcotest.(check bytes_opt) "good commit survived torn tail"
        (Some (bs "1")) v;
      let* () = S.close st2 in
      Lwt.return_unit
...
let test_seeded_random_truncation () =
  run (
    let n_seeds = 50 in
    let rec loop seed =
      if seed = n_seeds then Lwt.return_unit
      else begin
        Random.init seed;
        let path = fresh_path () in
        cleanup path;
        Lwt.finalize (fun () ->
          ...
          let fd =
            Unix.openfile (path ^ "-wal") [Unix.O_RDWR] 0o644
          in
          ...
          in
          if not (List.exists matches !prefixes) then
            Alcotest.failf "seed %d: torn WAL revealed non-prefix state"
              seed;
          Lwt.return_unit
        ) (fun () -> cleanup path; Lwt.return_unit)
        |> fun work ->
...
let () =
  Alcotest.run "wal_fault_injection" [
    "fault", [
