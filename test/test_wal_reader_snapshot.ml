...
let count_lwt db sql =
  ...
  | Ok stream -> Lwt_stream.to_list stream >|= List.length
;;
    ...
    (let* sr = S.open_file_wal ~path () in
     ...
     let rec seed i =
       ...
       then Lwt.return_unit
         ...
         let* () =
           S.put tx tid (bs (Printf.sprintf "k%04d" i)) (bs (Printf.sprintf "v%04d" i))
         in
         seed (i + 1)
     in
     let* () = seed 0 in
     ...
     let writer =
       let rec loop i =
         ...
         then Lwt.return_unit
           ...
           let* tx = S.rw_begin st in
           let* () =
             S.put tx tid (bs (Printf.sprintf "x%04d" i)) (bs (Printf.sprintf "y%04d" i))
           in
           let* () = S.commit tx in
           let* () = Lwt.pause () in
           loop (i + 1)
     ...
     Alcotest.(check int) "fresh snapshot sees all writes" (n_seed + n_extra) fresh;
     let* () = S.ro_end ro2 in
     let* () = S.close st in
     Lwt.return_unit);
  (try Unix.unlink path with
...
let () =
  Alcotest.run
    ...
    [ ( "snapshot"
      , [ Alcotest.test_case
            "concurrent reader sees monotonic count"
            `Slow
            test_concurrent_reader_sees_monotonic_count
        ; Alcotest.test_case
            "within-snapshot stable count under writes"
            `Quick
            test_within_snapshot_stable_count
        ] )
    ]
;;
