    ...
    (let* meta_opt = Cat.find_table cat ~name:table_name in
     ...
     Exec.execute
       ...
       (Plan.Op_insert
          { table_meta
          ; ordinals
          ; values = [ List.map (fun l -> Plan.P_lit l) values ]
          ; on_conflict = None
          ; returning = []
          ; upsert_update = None
          }))
...
let execute_with_count_pragma_returns_zero () =
  ...
  run
    (let* n =
       Exec.execute_with_count
         store
         cat
         (Plan.Op_pragma_rows { rows = [ [| Row.V_text "foo" |] ] })
     in
...
let exec_create_index_unknown_table () =
  ...
  run
    (let* () =
     ...
     in
     Alcotest.(check bool) "create_index on unknown table fails" true (outcome = `Err);
     Lwt.return_unit)
;;
...
let exec_drop_table_basic () =
  ...
  run
    (let* () =
     ...
     let m = Option.get meta_opt in
     let* () =
       Exec.execute store cat (Plan.Op_drop_table { table_meta = m; indexes = [] })
     in
...
let exec_drop_table_raises_in_query () =
  ...
  run
    (let* () =
     ...
     (try
        ignore
          (Exec.query store cat (Plan.Op_drop_table { table_meta = m; indexes = [] }));
...
let exec_drop_index_basic () =
  ...
  run
    (let* () =
     ...
     let* () = Exec.execute store cat (Plan.Op_drop_index { idx_info = idx }) in
     Alcotest.(check bool)
       "index gone after drop"
       true
       (Option.is_none (Cat.find_index cat ~name:"idx"));
     Lwt.return_unit)
;;
...
let exec_drop_index_returns_zero () =
  ...
  run
    (let* () =
     ...
     let idx = Option.get idx_opt in
     let* n = Exec.execute_with_count store cat (Plan.Op_drop_index { idx_info = idx }) in
...
let exec_drop_index_raises_in_query () =
  ...
  run
    (let* () =
     ...
     let idx = Option.get idx_opt in
     (try
        ignore (Exec.query store cat (Plan.Op_drop_index { idx_info = idx }));
...
let query_aggregate_sum_real () =
  ...
  run
    (let schema =
     ...
     (match (List.hd rows).(0) with
      | Row.V_real f ->
        Alcotest.(check bool) "sum_real = 4.0" true (abs_float (f -. 4.0) < 1e-9)
...
let query_aggregate_avg () =
  ...
  run
    (let* () =
     ...
     (match (List.hd rows).(0) with
      | Row.V_real f ->
        Alcotest.(check bool) "avg = 15.0" true (abs_float (f -. 15.0) < 1e-9)
...
let query_aggregate_count_col_skips_null () =
  ...
  run
    (let* () =
     ...
     in
     insert store cat "t" ([ 0; 1 ], [ Ast.L_int 1L; Ast.L_text "a" ]);
     insert store cat "t" ([ 1 ], [ Ast.L_text "b" ]);
     (* id = NULL *)
     insert store cat "t" ([ 0; 1 ], [ Ast.L_int 3L; Ast.L_text "c" ]);
     let* meta_opt = Cat.find_table cat ~name:"t" in
     let m = Option.get meta_opt in
  ...
  Alcotest.run
    ...
    ; ( "filter"
      , [ Alcotest.test_case "query_filter_eq_int" `Quick query_filter_eq_int
        ...
        ; Alcotest.test_case
            "query_filter_nonnull_eq_null"
            `Quick
            query_filter_nonnull_eq_null
        ; Alcotest.test_case
            "query_filter_type_mismatch"
            `Quick
            query_filter_type_mismatch
        ] )
    ; ( "project"
    ...
    ; ( "error_conditions"
      , [ Alcotest.test_case "execute_read_op_raises" `Quick execute_read_op_raises
        ...
        ; Alcotest.test_case
            "execute_with_count_pragma_zero"
            `Quick
            execute_with_count_pragma_returns_zero
        ] )
    ; ( "sort"
    ...
    ; ( "index_lookup"
      , [ Alcotest.test_case "query_index_lookup_basic" `Quick query_index_lookup_basic
        ; Alcotest.test_case
            "query_index_lookup_type_mismatch"
            `Quick
            query_index_lookup_type_mismatch
        ; Alcotest.test_case
            "query_index_lookup_multiple"
            `Quick
            query_index_lookup_multiple_matches
        ...
        ; Alcotest.test_case
            "exec_create_index_unknown_table"
            `Quick
            exec_create_index_unknown_table
        ; Alcotest.test_case
            "unique_first_insert_succeeds"
            `Quick
            unique_index_first_insert_succeeds
        ] )
    ; ( "update"
      , [ Alcotest.test_case
            "exec_update_no_match_returns_zero"
            `Quick
            exec_update_no_match_returns_zero
        ; Alcotest.test_case
            "exec_update_match_returns_count"
            `Quick
            exec_update_match_returns_count
        ; Alcotest.test_case
            "exec_update_raises_in_query"
            `Quick
            exec_update_raises_in_query
        ] )
    ; ( "delete"
      , [ Alcotest.test_case
            "exec_delete_no_match_returns_zero"
            `Quick
            exec_delete_no_match_returns_zero
        ; Alcotest.test_case
            "exec_delete_all_returns_count"
            `Quick
            exec_delete_all_returns_count
        ; Alcotest.test_case
            "exec_delete_raises_in_query"
            `Quick
            exec_delete_raises_in_query
        ] )
    ; ( "drop"
      , [ Alcotest.test_case "exec_drop_table_basic" `Quick exec_drop_table_basic
        ; Alcotest.test_case
            "exec_drop_table_returns_zero"
            `Quick
            exec_drop_table_returns_zero
        ; Alcotest.test_case
            "exec_drop_table_raises_in_query"
            `Quick
            exec_drop_table_raises_in_query
        ; Alcotest.test_case "exec_drop_index_basic" `Quick exec_drop_index_basic
        ; Alcotest.test_case
            "exec_drop_index_returns_zero"
            `Quick
            exec_drop_index_returns_zero
        ; Alcotest.test_case
            "exec_drop_index_raises_in_query"
            `Quick
            exec_drop_index_raises_in_query
        ] )
    ; ( "hash_join"
      , [ Alcotest.test_case "query_hash_join_inner" `Quick query_hash_join_inner
        ...
        ; Alcotest.test_case
            "query_hash_join_null_key"
            `Quick
            query_hash_join_null_key_excluded
        ; Alcotest.test_case
            "query_nlj_raises_in_execute"
            `Quick
            query_nlj_raises_in_execute
        ; Alcotest.test_case
            "query_hash_join_raises_in_execute"
            `Quick
            query_hash_join_raises_in_execute
        ] )
    ...
    ; ( "aggregate"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "query_aggregate_count_col_skips_null"
            `Quick
            query_aggregate_count_col_skips_null
        ; Alcotest.test_case
            "query_aggregate_with_group_by"
            `Quick
            query_aggregate_with_group_by
        ; Alcotest.test_case
            "query_aggregate_with_having"
            `Quick
            query_aggregate_with_having
        ; Alcotest.test_case
            "query_aggregate_raises_in_execute"
            `Quick
            query_aggregate_raises_in_execute
        ] )
    ]
