...
let bytes_opt =
  Alcotest.(option
    (testable
       (fun ppf b ->
          Format.fprintf ppf "<%d bytes h=%d>"
            (Bytes.length b) (Hashtbl.hash b))
       Bytes.equal))
...
let test_multi_page_overflow () =
  run (
    ...
    Lwt.finalize (fun () ->
      ...
      let* () = S.ro_end tx2 in
      let* () = S.close st in
      Alcotest.(check bytes_opt) "12000-byte value round-trip"
        (Some v) g;
      Lwt.return_unit
    ) (fun () -> cleanup path; Lwt.return_unit))
...
let test_inline_does_not_overflow () =
  run (
    ...
    Lwt.finalize (fun () ->
      ...
      Alcotest.(check bool)
        (Printf.sprintf "inline put uses fewer pages than overflow (inline=%Ld, ovf=%Ld)"
           inline_pages overflow_pages)
        true (Int64.compare inline_pages overflow_pages < 0);
      Alcotest.(check bool)
        (Printf.sprintf "single inline put fits in ≤6 pages (got %Ld)"
           inline_pages)
        true (Int64.compare inline_pages 7L < 0);
      Lwt.return_unit
    ) (fun () -> cleanup p1; cleanup p2; Lwt.return_unit))
  ...
  run (
    ...
    Lwt.finalize (fun () ->
      ...
      (* File should not have grown by more than ~3x — chains get freed
         ...
         with every replace. *)
      Alcotest.(check bool) "file size grew bounded"
        true (size_after_third <= size_after_first * 4);
      let* () = S.close st in
      Lwt.return_unit
    ) (fun () -> cleanup path; Lwt.return_unit))
...
let test_del_frees_chain () =
  run (
    ...
    Lwt.finalize (fun () ->
      ...
      Alcotest.(check bool) "file did not balloon over 5 del/put cycles"
        true (final_size <= !initial_size * 3);
      let* () = S.close st in
      Lwt.return_unit
    ) (fun () -> cleanup path; Lwt.return_unit))
...
let test_cursor_decodes_overflow () =
  run (
    ...
    Lwt.finalize (fun () ->
      ...
      Alcotest.(check bool) "cursor returns decoded 10KB value"
        true (Bytes.equal vb big);
      Lwt.return_unit
    ) (fun () -> cleanup path; Lwt.return_unit))
...
let test_savepoint_rollback_overflow () =
  run (
    ...
    Lwt.finalize (fun () ->
      ...
      let* () = S.ro_end tx_ro in
      let* () = S.close st in
      Alcotest.(check bytes_opt) "rollback restored pre-savepoint value"
        (Some pre) g;
      Lwt.return_unit
    ) (fun () -> cleanup path; Lwt.return_unit))
...
let test_page_codec_overflow () =
  ...
  Page.write_overflow buf ~next_pid:42l ~payload
    ~payload_off:0 ~payload_len:(Bytes.length payload);
  Page.seal buf;
  ...
  Alcotest.(check int) "right_page = 42" 42
    (Int32.to_int common.right_page);
  Alcotest.(check int) "payload_len" 100 (Page.overflow_payload_len buf);
  Alcotest.(check bool) "payload bytes match"
    true (Bytes.equal payload (Page.overflow_payload buf))

let () =
  Alcotest.run "overflow" [
    "page-codec", [
