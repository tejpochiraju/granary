...
let reader_workload st ~read_ops =
  ...
  let rec loop i =
    if i >= read_ops then Lwt.return_unit
    else
...
let baseline_run st ~n_commits ~n_readers ~read_ops =
  ...
  let readers =
    List.init n_readers (fun _ -> reader_workload st ~read_ops)
  in
...
let run_config ~delay ~n_seed mode ~n_commits ~n_readers ~read_ops =
  ...
  run (
    ...
    let* (w, r) =
      match mode with
      | `Baseline ->
        baseline_run st ~n_commits ~n_readers ~read_ops
      | `Parallel ->
        parallel_run st ~n_commits ~n_readers ~read_ops
    in
...
let () =
  Alcotest.run "wal_fsync_overlap" [
    "bench", [
      Alcotest.test_case "parallel reads overlap writer fsync"
        `Slow test_fsync_overlap;
    ]
  ]
