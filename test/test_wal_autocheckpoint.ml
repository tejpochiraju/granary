...
let wal_size path =
  try (Unix.stat (path ^ "-wal")).Unix.st_size with _ -> 0
...
let test_bounded_under_low_threshold () =
  run @@ with_fresh ~f:(fun path ->
    ...
    let st = match sr with Ok t -> t | Error e ->
      Alcotest.failf "open_file_wal: %a" S.pp_error e
    in
    ...
    let rec loop i =
      if i = 500 then Lwt.return_unit
      else
        let* tx = S.rw_begin st in
        let* () = S.put tx 16 (bs (Printf.sprintf "k%04d" i))
                    (bs (Printf.sprintf "v%04d" i)) in
        let* () = S.commit tx in
        loop (i + 1)
    in
    let* () = loop 0 in
    ...
    let* () = S.ro_end tx in
    Alcotest.(check (option string)) "data still readable"
      (Some "v0250")
...
let test_default_threshold () =
  run @@ with_fresh ~f:(fun path ->
    ...
    let st = match sr with Ok t -> t | Error e ->
      Alcotest.failf "open_file_wal: %a" S.pp_error e
    in
    Alcotest.(check int) "default threshold = 1000" 1000
      (S.wal_autocheckpoint st);
    let* () = S.close st in
    Lwt.return_unit)
...
let test_pragma_round_trip () =
  run @@ with_fresh ~f:(fun path ->
    ...
    (match r with Ok () -> () | Error e ->
       Alcotest.failf "execute set: %a" D.pp_error e);
    let* s = D.query db "PRAGMA wal_autocheckpoint" in
    let s = match s with Ok s -> s | Error e ->
      Alcotest.failf "query: %a" D.pp_error e
    in
    ...
    (match r with Ok () -> () | Error e ->
       Alcotest.failf "execute disable: %a" D.pp_error e);
    let* s = D.query db "PRAGMA wal_autocheckpoint" in
    let s = match s with Ok s -> s | Error e ->
      Alcotest.failf "query: %a" D.pp_error e
    in
