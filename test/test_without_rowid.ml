...
let row_to_string row =
  String.concat "|" (List.map value_to_string (Array.to_list row))
...
let test_create_basic () =
  run (
    ...
    let* () = exec db
      "CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT) WITHOUT ROWID" in
    ...
    Alcotest.(check string) "first row by id"
      "10|bar" (row_to_string (List.nth rows 0));
    Alcotest.(check string) "second row by id"
      "42|foo" (row_to_string (List.nth rows 1));
    let* () = Db.close db in
    cleanup path;
    Lwt.return_unit)
  ...
  run (
    ...
    let* failed =
      Lwt.catch
        (fun () ->
          let* () = exec db
            "CREATE TABLE t (k TEXT PRIMARY KEY, v INT) WITHOUT ROWID" in
          Lwt.return_false)
        (fun _ -> Lwt.return_true)
    in
...
let test_null_pk_rejected () =
  run (
    ...
    let* db = open_path path in
    let* () = exec db
      "CREATE TABLE t (id INTEGER PRIMARY KEY, x INT) WITHOUT ROWID" in
    let* failed =
      Lwt.catch
        (fun () ->
...
let test_persistence () =
  run (
    ...
    Alcotest.(check string) "row 0" "7|lucky"
      (row_to_string (List.nth rows 0));
    Alcotest.(check string) "row 1" "8|eight"
      (row_to_string (List.nth rows 1));
    let* () = Db.close db2 in
    cleanup path;
    Lwt.return_unit)
...
let test_update_and_delete () =
  run (
    ...
    Alcotest.(check string) "remaining row" "6|Y"
      (row_to_string (List.nth rows 0));
    let* () = Db.close db in
    cleanup path;
    Lwt.return_unit)
