...
let sqlite3_run_setup ~db_path stmts =
  ...
  | _ ->
    ...
    let cmd =
      Printf.sprintf
        "sqlite3 %s %s >/dev/null 2>&1"
        (Filename.quote db_path)
        (Filename.quote batch)
    in
...
let sqlite3_run_query ~db_path query =
  ...
  List.filter_map
    (fun line ->
       if String.length line = 0 then None else Some (String.split_on_char '\t' line))
    lines
;;
...
let cases =
  ...
  ; { name = "having_sum"
    ...
    ; query =
        "SELECT cat, SUM(val) FROM t GROUP BY cat HAVING SUM(val) > 5 ORDER BY cat ASC"
    ; unordered = false
    }
  ; (* LIMIT / OFFSET *)
    { name = "limit"
  ...
  ; (* JOINs *)
    ...
    ; query =
        "SELECT a.name, b.val FROM a INNER JOIN b ON a.id = b.aid ORDER BY b.val ASC"
    ; unordered = false
    }
  ...
  ; { name = "join_null_key_excluded"
    ...
    ; query =
        "SELECT a.name, b.val FROM a INNER JOIN b ON a.id = b.aid ORDER BY a.name ASC"
    ; unordered = false
    }
  ...
  ; (* HAVING with qualified table.col inside aggregate function *)
    ...
    ; query =
        "SELECT cat, SUM(val) FROM t GROUP BY cat HAVING SUM(t.val) > 5 ORDER BY cat ASC"
    ; unordered = false
    }
  ; (* ORDER BY with qualified column in JOIN query *)
    ...
    ; query =
        "SELECT a.name, b.val FROM a INNER JOIN b ON a.id = b.aid ORDER BY a.name DESC"
    ; unordered = false
    }
  ...
  ; (* Phase 7: date/time functions *)
    ...
    ; setup = [ "CREATE TABLE _d (x INTEGER)"; "INSERT INTO _d VALUES (1)" ]
  ...
  ; { name = "update_returning_new_values"
    ; setup =
        [ "CREATE TABLE t (id INTEGER, score INTEGER)"; "INSERT INTO t VALUES (1, 100)" ]
...
let phase9_subquery_cases =
  ...
  ; { name = "exists_false"
    ; setup = [ "CREATE TABLE empty (id INTEGER PRIMARY KEY)" ]
  ...
  ; { name = "from_less_select"; setup = []; query = "SELECT 1 + 1"; unordered = false }
  ]
;;
...
let sqlite3_last_setup_fails ~db_path setup =
  ...
  let cmd =
    Printf.sprintf "sqlite3 %s %s 2>&1" (Filename.quote db_path) (Filename.quote last)
  in
  let ic = Unix.open_process_in cmd in
  let output = ref [] in
  (try
...
let run_check_error_case ce =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       then
         Alcotest.failf
           "%s: sqlite3 did not raise an error (expected CHECK violation)"
           ce.ce_name)
;;
...
let phase9_check_cases =
  ...
  ; { name = "check_compound_expr"
    ; setup =
        [ "CREATE TABLE t (id INTEGER PRIMARY KEY, price REAL CHECK (price > 0 AND price \
           < 10000))"
...
let phase10_multi_join_cases =
  ...
  ; { name = "three_join_count"
    ...
    ; query =
        "SELECT dept.dname, COUNT(emp.id) FROM dept JOIN emp ON dept.id = emp.dept_id \
         JOIN sal ON emp.id = sal.emp_id GROUP BY dname ORDER BY dname"
    ; unordered = false
    }
  ; { name = "case_in_multi_join"
    ...
    ; query =
        "SELECT a.id, CASE WHEN b.v > 15 THEN 'big' ELSE 'small' END FROM a JOIN b ON \
         a.id = b.aid ORDER BY a.id"
    ; unordered = false
    }
  ]
;;
...
let phase11_alias_cases =
  ...
  ; { name = "col_alias_multiple"
    ; setup = [ "CREATE TABLE t (a INTEGER, b INTEGER)"; "INSERT INTO t VALUES (2, 3)" ]
...
let phase12_cte_cases =
  ...
  ; { name = "cte_agg"
    ...
    ; query =
        "WITH totals AS (SELECT uid, SUM(amt) AS total FROM orders GROUP BY uid) SELECT \
         * FROM totals ORDER BY uid"
    ; unordered = false
    }
  ; { name = "cte_order"
    ; setup =
...
let phase12_multirow_cases =
  ...
  ; { name = "multirow_single"
    ; setup = [ "CREATE TABLE t (x INTEGER)"; "INSERT INTO t VALUES (42)" ]
    ; query = "SELECT * FROM t"
    ; unordered = false
    }
  ; { name = "multirow_count"
...
let phase12_correlated_cases =
  [ { name = "corr_exists"
    ...
    ; query =
        "SELECT name FROM u WHERE EXISTS (SELECT 1 FROM o WHERE o.uid = u.id) ORDER BY \
         name"
    ; unordered = false
    }
  ; { name = "corr_not_exists"
    ...
    ; query =
        "SELECT name FROM u WHERE NOT EXISTS (SELECT 1 FROM o WHERE o.uid = u.id) ORDER \
         BY name"
    ; unordered = false
    }
  ; { name = "corr_in_select"
    ...
    ; query =
        "SELECT name FROM u WHERE u.id IN (SELECT uid FROM o WHERE o.uid = u.id) ORDER \
         BY name"
    ; unordered = false
    }
  ; { name = "corr_not_in"
    ...
    ; query =
        "SELECT name FROM u WHERE u.id NOT IN (SELECT uid FROM blocked WHERE blocked.uid \
         = u.id) ORDER BY name"
    ; unordered = false
    }
  ]
...
let phase13_upsert_cases =
  ...
  ; { name = "upsert_no_conflict"
    ; setup =
        ...
        ; "INSERT INTO kv VALUES (2, 'second') ON CONFLICT(k) DO UPDATE SET v = \
           excluded.v"
        ]
  ...
  ; { name = "upsert_increment"
    ; setup =
        ...
        ; "INSERT INTO counters VALUES ('hits', 0) ON CONFLICT(name) DO UPDATE SET cnt = \
           cnt + 1"
        ]
  ...
  ; { name = "upsert_multi_assign"
    ; setup =
        ...
        ; "INSERT INTO t VALUES (1, 'a2', 'b2') ON CONFLICT(id) DO UPDATE SET a = \
           excluded.a, b = excluded.b"
        ]
...
let phase13_view_cases =
  ...
  ; { name = "view_aggregation"
    ; setup =
        ...
        ; "CREATE VIEW order_counts AS SELECT customer, COUNT(*) AS cnt FROM orders \
           GROUP BY customer"
        ]
...
let phase14_window_cases =
  ...
  ; { name = "window_partition_sum"
    ...
    ; query =
        "SELECT dept, v, SUM(v) OVER (PARTITION BY dept) AS dept_total FROM t ORDER BY \
         dept, v"
    ; unordered = false
    }
  ; { name = "window_count_star"
    ; setup = [ "CREATE TABLE t (x INTEGER)"; "INSERT INTO t VALUES (3),(1),(2)" ]
...
let phase14_recursive_cte_cases =
  [ { name = "recursive_series"
    ; setup = []
    ; query =
        "WITH RECURSIVE cnt AS (SELECT 1 AS n UNION ALL SELECT n + 1 FROM cnt WHERE n < \
         5) SELECT n FROM cnt ORDER BY n"
    ; unordered = false
    }
  ; { name = "recursive_sum"
    ; setup = []
    ; query =
        "WITH RECURSIVE nums AS (SELECT 1 AS i, 1 AS s UNION ALL SELECT i + 1, s + (i + \
         1) FROM nums WHERE i < 4) SELECT s FROM nums WHERE i = 4"
    ; unordered = false
    }
  ; { name = "recursive_tree"
    ; setup =
    ...
    ; query =
        "WITH RECURSIVE anc AS (SELECT id FROM tree WHERE parent = 1 UNION ALL SELECT \
         t.id FROM tree AS t INNER JOIN anc AS a ON t.parent = a.id) SELECT id FROM anc \
         ORDER BY id"
    ; unordered = false
    }
  ; { name = "recursive_fibonacci"
    ; setup = []
    ; query =
        "WITH RECURSIVE fib AS (SELECT 0 AS a, 1 AS b UNION ALL SELECT b, a + b FROM fib \
         WHERE a < 10) SELECT a FROM fib ORDER BY a"
    ; unordered = false
    }
  ]
;;
...
let phase17_frame_cases =
  [ { name = "rows_1_preceding_sum"
    ...
    ; query =
        "SELECT n, SUM(n) OVER (ORDER BY n ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) AS \
         s FROM fr1 ORDER BY n"
    ; unordered = false
    }
  ...
  ; { name = "rows_centered_sum"
    ...
    ; query =
        "SELECT n, SUM(n) OVER (ORDER BY n ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS \
         s FROM fr3 ORDER BY n"
    ; unordered = false
    }
  ]
;;
...
let phase17_pctrank_cases =
  [ { name = "percent_rank"
    ...
    ; query =
        "SELECT s, CAST(ROUND(PERCENT_RANK() OVER (ORDER BY s), 3) AS TEXT) AS pr FROM \
         pr1 ORDER BY s"
    ; unordered = false
    }
  ; { name = "cume_dist"
    ...
    ; query =
        "SELECT s, CAST(ROUND(CUME_DIST() OVER (ORDER BY s), 2) AS TEXT) AS cd FROM cd1 \
         ORDER BY s"
    ; unordered = false
    }
  ; { name = "percent_rank_desc"
    ...
    ; query =
        "SELECT s, CAST(ROUND(PERCENT_RANK() OVER (ORDER BY s DESC), 3) AS TEXT) AS pr \
         FROM pr2 ORDER BY s DESC"
    ; unordered = false
    }
  ]
;;
...
let phase18_multigroup_cases =
  [ (* Two-column GROUP BY with COUNT *)
    ...
    ; query =
        "SELECT dept, role, COUNT(*) FROM mg1 GROUP BY dept, role ORDER BY dept, role"
    ; unordered = false
    }
  ...
  ; (* Two-column GROUP BY with HAVING *)
    ...
    ; query =
        "SELECT dept, role, COUNT(*) FROM mg3 GROUP BY dept, role HAVING COUNT(*) > 1 \
         ORDER BY dept, role"
    ; unordered = false
    }
...
let phase19_nulls_cases =
  let setup =
    [ "CREATE TABLE tn (x INTEGER)"
    ; "INSERT INTO tn VALUES (1)"
    ; "INSERT INTO tn VALUES (NULL)"
    ; "INSERT INTO tn VALUES (3)"
    ]
  in
  [ { name = "nulls last asc"
    ; setup
    ; query = "SELECT x FROM tn ORDER BY x ASC NULLS LAST"
    ; unordered = false
    }
  ; { name = "nulls first asc"
    ; setup
    ; query = "SELECT x FROM tn ORDER BY x ASC NULLS FIRST"
    ; unordered = false
    }
  ; { name = "nulls first desc"
    ; setup
    ; query = "SELECT x FROM tn ORDER BY x DESC NULLS FIRST"
    ; unordered = false
    }
  ; { name = "nulls last desc"
    ; setup
    ; query = "SELECT x FROM tn ORDER BY x DESC NULLS LAST"
    ; unordered = false
    }
  ]
;;
...
let phase19_window_agg_cases =
  ...
  ; { name = "dense_rank over count"
    ...
    ; query =
        "SELECT dept, COUNT(*), DENSE_RANK() OVER (ORDER BY COUNT(*) DESC) FROM wagg \
...
let phase22_trigger_cases =
  ...
  ; { name = "trigger_when_clause"
    ; setup =
        ...
        ; "CREATE TRIGGER t_when AFTER INSERT ON t WHEN NEW.score > 100 BEGIN INSERT \
           INTO high_scores VALUES (NEW.id); END"
        ; "INSERT INTO t VALUES (1, 50)"
  ...
  ; { name = "drop_trigger"
    ; setup =
        ...
        ; "CREATE TRIGGER t_ai AFTER INSERT ON t BEGIN INSERT INTO audit VALUES \
           (NEW.id); END"
        ; "INSERT INTO t VALUES (1)"
        ; "DROP TRIGGER t_ai"
        ; "INSERT INTO t VALUES (2)"
...
let phase23_cascade_cases =
  [ { name = "cascade_delete_basic"
    ; setup =
        ...
        ; "CREATE TABLE chi (id INTEGER, pid INTEGER REFERENCES par(id) ON DELETE \
           CASCADE)"
  ...
  ; { name = "cascade_delete_all"
    ; setup =
        ...
        ; "CREATE TABLE chi2 (id INTEGER, pid INTEGER REFERENCES par2(id) ON DELETE \
           CASCADE)"
  ...
  ; { name = "cascade_update"
    ; setup =
        ...
        ; "CREATE TABLE chi3 (id INTEGER, pid INTEGER REFERENCES par3(id) ON UPDATE \
           CASCADE)"
  ...
  ; { name = "set_null_on_delete"
    ; setup =
        ...
        ; "CREATE TABLE chi4 (id INTEGER, pid INTEGER REFERENCES par4(id) ON DELETE SET \
           NULL)"
  ...
  ; { name = "cascade_table_level_fk"
    ; setup =
        ...
        ; "CREATE TABLE chi5 (id INTEGER, pid INTEGER, FOREIGN KEY (pid) REFERENCES \
           par5(id) ON DELETE CASCADE)"
...
let phase30_cascade_cases =
  [ { name = "cascade_delete_three_levels"
    ; setup =
        ...
        ; "CREATE TABLE b (id INTEGER PRIMARY KEY, aid INTEGER REFERENCES a(id) ON \
           DELETE CASCADE)"
  ...
  ; { name = "cascade_update_three_levels"
    ; setup =
        [ "PRAGMA foreign_keys = 1"
        ; "CREATE TABLE a (id INTEGER PRIMARY KEY)"
        ; "CREATE TABLE b (bid INTEGER PRIMARY KEY, aid INTEGER, UNIQUE (aid), FOREIGN \
           KEY (aid) REFERENCES a(id) ON UPDATE CASCADE)"
        ; "CREATE TABLE c (cid INTEGER, c_ref INTEGER REFERENCES b(aid) ON UPDATE \
           CASCADE)"
        ; "INSERT INTO a VALUES (1)"
        ; "INSERT INTO b VALUES (10, 1)"
...
let phase30_sqlite_master_cases =
  [ { name = "sqlite_master_table_type"
    ...
    ; query = "SELECT type, name FROM sqlite_master WHERE type='table' AND name='users'"
    ; unordered = false
    }
  ; { name = "sqlite_master_index_type"
    ; setup = [ "CREATE TABLE t (id INTEGER, v TEXT)"; "CREATE INDEX idx_v ON t (v)" ]
    ; query = "SELECT type, name, tbl_name FROM sqlite_master WHERE type='index'"
    ; unordered = false
    }
  ; { name = "sqlite_master_count_tables"
    ; setup = [ "CREATE TABLE a (id INTEGER)"; "CREATE TABLE b (id INTEGER)" ]
  ...
  ; { name = "sqlite_master_view_type"
    ; setup = [ "CREATE TABLE t (id INTEGER)"; "CREATE VIEW v AS SELECT id FROM t" ]
...
let phase35_fk_deferrable_cases =
  [ { name = "deferred_child_before_parent_commits"
    ; setup =
        ...
        ; "CREATE TABLE chi35 (id INTEGER PRIMARY KEY, pid INTEGER REFERENCES par35(id) \
  ...
  ; { name = "pragma_defer_foreign_keys_overrides"
    ; setup =
        ...
        ; "CREATE TABLE chi35b (id INTEGER PRIMARY KEY, pid INTEGER REFERENCES \
           par35b(id))"
        ; "BEGIN"
  ...
  ; { name = "deferred_resolved_by_child_delete"
    ; setup =
        ...
        ; "CREATE TABLE chi35c (id INTEGER PRIMARY KEY, pid INTEGER REFERENCES \
           par35c(id) DEFERRABLE INITIALLY DEFERRED)"
        ; "BEGIN"
...
let phase34_master_fts_cases =
  ...
  ; { name = "sqlite_master_fts_and_regular_both_table_type"
    ...
    ; query =
        "SELECT name FROM sqlite_master WHERE type='table' AND (name='regular' OR \
         name='search') ORDER BY name"
    ; unordered = false
    }
  ; { name = "sqlite_master_count_includes_fts"
    ; setup = [ "CREATE TABLE a (id INT)"; "CREATE VIRTUAL TABLE b USING fts5(x)" ]
    ; query =
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND (name='a' OR name='b')"
    ; unordered = false
    }
  ]
;;
...
let phase31_scalar_cases =
  ...
  ; { name = "hex_text"; setup = []; query = "SELECT HEX('abc')"; unordered = false }
  ; { name = "hex_null"
    ; setup = []
    ; query = "SELECT TYPEOF(HEX(NULL))"
    ; unordered = false
    }
  ; { name = "hex_int_zero"; setup = []; query = "SELECT HEX(0)"; unordered = false }
  ; { name = "hex_int_nonzero"; setup = []; query = "SELECT HEX(255)"; unordered = false }
  ; { name = "char_basic"
    ; setup = []
    ; query = "SELECT CHAR(65, 66, 67)"
    ; unordered = false
    }
  ; { name = "char_single"; setup = []; query = "SELECT CHAR(65)"; unordered = false }
...
let phase32_multi_col_fk_cases =
  ...
  ; { name = "multi_fk_null_in_col_allowed"
    ; setup =
        [ "PRAGMA foreign_keys = 1"
        ; "CREATE TABLE parent (x INTEGER, y INTEGER, PRIMARY KEY (x, y))"
        ; "CREATE TABLE child (id INTEGER, pa INTEGER, pb INTEGER, FOREIGN KEY (pa, pb) \
...
let phase33_trigger_cases =
  [ (* Note: SQLite by default does NOT fire DELETE triggers on the rows
    ...
    ; setup =
        ...
        ; "CREATE TRIGGER tb AFTER INSERT ON b BEGIN INSERT INTO c(s) VALUES(NEW.m); END"
...
let phase33_virtual_gen_cases =
  ...
  ; { name = "virtual_gen_after_update"
    ; setup =
        [ "CREATE TABLE t (id INT, a INT, b INT, c INT GENERATED ALWAYS AS (a + b) \
           VIRTUAL)"
...
let phase35_snippet_parity_cases =
  ...
  ; { name = "snippet_sentence_lede_bonus"
    ; setup =
        [ "CREATE VIRTUAL TABLE t USING fts5(c)"
        ; "INSERT INTO t VALUES('aaa bbb ccc foo a b c d e f g h i j foo bar foo baz \
           qux')"
        ]
  ...
  ; { name = "snippet_dense_cluster_wins"
    ; setup =
        [ "CREATE VIRTUAL TABLE t USING fts5(c)"
        ; "INSERT INTO t VALUES('w1 w2 w3 w4 w5 w6 w7 w8 w9 wa wb wc wd we wf foo bar \
           foo baz qux')"
        ]
...
let () =
  Alcotest.run
    ...
    ; ( "phase9_check"
      , List.map make_test phase9_check_cases
