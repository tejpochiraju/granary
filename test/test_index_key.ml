...
let test_null_less_than_int_zero () =
  ...
  if not (cmp_bytes enc_null enc_zero < 0)
  then Alcotest.fail "NULL should sort before IK_int 0L"
;;
...
let test_int_ordering_min_zero_max () =
  ...
  if not (cmp_bytes enc_min enc_zero < 0)
  then Alcotest.fail "IK_int min_int should sort before IK_int 0L";
  if not (cmp_bytes enc_zero enc_max < 0)
  then Alcotest.fail "IK_int 0L should sort before IK_int max_int"
;;

let test_null_less_than_min_int () =
  let enc_null = encode_value IK_null in
  let enc_min = encode_value (IK_int Int64.min_int) in
  if not (cmp_bytes enc_null enc_min < 0)
  then Alcotest.fail "NULL should sort before IK_int min_int"
;;
...
let test_real_ordering_neg_zero_pos () =
  ...
  if not (cmp_bytes enc_neg enc_z < 0)
  then Alcotest.fail "IK_real -1.0 should sort before IK_real 0.0";
  if not (cmp_bytes enc_z enc_pos < 0)
...
let test_text_ordering () =
  ...
  if not (cmp_bytes enc_a enc_aa < 0)
  then Alcotest.fail "IK_text \"a\" should sort before IK_text \"aa\"";
  if not (cmp_bytes enc_aa enc_b < 0)
  then Alcotest.fail "IK_text \"aa\" should sort before IK_text \"b\""
;;
...
let test_text_embedded_null_roundtrip () =
  ...
  (match decode (encode [ IK_text s ] ~rowid:0L) with
   ...
   | Ok ([ IK_text s' ], 0L) -> Alcotest.(check string) "text with null" s s'
  ...
  if Bytes.length enc <= 1 + raw_len
  then Alcotest.fail "expected escaping to increase length"
;;
...
let test_text_embedded_null_order () =
  ...
  (* "a\x00" < "a\x00b" *)
  if not (cmp_bytes a_null a_null_b < 0)
  then Alcotest.fail "\"a\\x00\" should sort before \"a\\x00b\"";
  ...
  if not (cmp_bytes a_null_b aa < 0)
  then Alcotest.fail "\"a\\x00b\" should sort before \"aa\""
;;
...
let test_blob_ordering () =
  ...
  if not (cmp_bytes enc_empty enc_one < 0)
  then Alcotest.fail "IK_blob \"\" should sort before IK_blob \"\\x01\""
;;
...
let test_roundtrip_text_embedded_null () =
  ...
  match decode (encode [ IK_text s ] ~rowid:0L) with
  | Error msg -> Alcotest.failf "decode error: %s" msg
  | Ok ([ IK_text s' ], 0L) -> Alcotest.(check string) "embedded null text" s s'
  | Ok _ -> Alcotest.fail "unexpected result"
;;
...
let test_decode_buffer_too_short () =
  ...
  | Error msg ->
    Alcotest.(check bool) "error mentions too short" true (String.length msg > 0)
...
let test_decode_missing_escape_terminator () =
  ...
  (* 'A' *)
  Bytes.set_uint8 buf 2 0x42;
  (* 'B' *)
  Bytes.set_uint8 buf 3 0x43;
  (* 'C' *)
  Bytes.set_uint8 buf 4 0x44;
  (* 'D' *)
  Bytes.set_uint8 buf 5 0x45;
  (* 'E' - no terminator, reaches rowid *)
...
let prop_int_order_preserving =
  QCheck.Test.make
    ~count:10000
    ~name:"int order preserving"
    (QCheck.pair QCheck.int64 QCheck.int64)
    (fun (a, b) ->
       let cmp_int = sign (Int64.compare a b) in
       let cmp_bytes =
         sign (Bytes.compare (encode_value (IK_int a)) (encode_value (IK_int b)))
       in
       cmp_int = cmp_bytes)
;;
...
let prop_text_order_preserving =
  QCheck.Test.make
    ~count:10000
    ~name:"text order preserving"
    (QCheck.pair QCheck.string QCheck.string)
    (fun (a, b) ->
       let cmp_str = sign (String.compare a b) in
       let cmp_bytes =
         sign (Bytes.compare (encode_value (IK_text a)) (encode_value (IK_text b)))
       in
       cmp_str = cmp_bytes)
;;
...
let prop_real_order_preserving =
  QCheck.Test.make
    ...
    (fun (a, b) ->
       ...
       let cmp_bytes =
         sign (Bytes.compare (encode_value (IK_real a)) (encode_value (IK_real b)))
       in
       cmp_float = cmp_bytes)
;;
...
let () =
  let unit_tests =
    [ ( "null"
      , [ Alcotest.test_case
            "encodes to 1 byte [0x00]"
            `Quick
            test_null_encodes_to_one_byte
    ...
    ; ( "integer"
      , [ Alcotest.test_case
            "min_int < 0L < max_int"
            `Quick
            test_int_ordering_min_zero_max
        ] )
    ; ( "real"
    ...
    ; ( "text"
      , [ Alcotest.test_case "\"a\" < \"aa\" < \"b\"" `Quick test_text_ordering
        ; Alcotest.test_case
            "embedded null roundtrip"
            `Quick
            test_text_embedded_null_roundtrip
        ; Alcotest.test_case "embedded null order" `Quick test_text_embedded_null_order
        ] )
    ; "blob", [ Alcotest.test_case "\"\" < \"\\x01\"" `Quick test_blob_ordering ]
    ; ( "cross_type"
      , [ Alcotest.test_case
            "NULL < INT < REAL < TEXT < BLOB"
            `Quick
            test_cross_type_ordering
        ] )
    ; ( "roundtrip"
      , [ Alcotest.test_case "IK_int 0L" `Quick test_roundtrip_int_zero
        ...
        ; Alcotest.test_case
            "text with embedded null"
            `Quick
            test_roundtrip_text_embedded_null
        ] )
    ; ( "decode_errors"
      , [ Alcotest.test_case "buffer too short" `Quick test_decode_buffer_too_short
        ; Alcotest.test_case
            ...
            `Quick
            test_decode_truncated_integer
        ; Alcotest.test_case "truncated REAL field" `Quick test_decode_truncated_real
        ; Alcotest.test_case
            "missing escape terminator"
            `Quick
            test_decode_missing_escape_terminator
        ...
        ; Alcotest.test_case
            "wrong rowid tail length"
            `Quick
            test_decode_rowid_tail_wrong_length
        ] )
    ]
  in
  let qcheck_tests =
