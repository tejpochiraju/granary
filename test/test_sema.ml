...
let bind_create_preserves_cols () =
  ...
  | Ok (Sema.BS_create_table { columns; _ }) ->
    ...
    (match (List.nth columns 0).Row.ty with
     | Row.Text -> ()
     | _ -> Alcotest.fail "col 0 should be Text");
    (match (List.nth columns 1).Row.ty with
     | Row.Integer -> ()
     | _ -> Alcotest.fail "col 1 should be Integer")
  | Ok _ -> Alcotest.fail "expected BS_create_table"
...
let bind_select_where_col_eq_lit () =
  ...
  | Ok
      (Sema.BS_select
         { where =
             Some (Sema.BE_binop (Sema.Eq, Sema.BE_col 0, Sema.BE_lit (Ast.L_int 42L)))
         ; _
         }) -> ()
...
let bind_select_where_lit_eq_col () =
  ...
  | Ok
      (Sema.BS_select
         { where =
             Some (Sema.BE_binop (Sema.Eq, Sema.BE_lit (Ast.L_int 42L), Sema.BE_col 0))
         ; _
         }) -> ()
  | Ok (Sema.BS_select { where = Some _; _ }) ->
    Alcotest.fail "where bound incorrectly (reversed)"
  | Ok _ -> Alcotest.fail "expected BS_select"
  | Error _ -> Alcotest.fail "unexpected error"
;;
...
let bind_select_where_text_col () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; joins = []
      ; where =
          Some (Ast.E_binop (Ast.Eq, Ast.E_col "name", Ast.E_lit (Ast.L_text "alice")))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
  ...
  | Ok
      (Sema.BS_select
         { where =
             Some
               (Sema.BE_binop (Sema.Eq, Sema.BE_col 1, Sema.BE_lit (Ast.L_text "alice")))
         ; _
         }) -> ()
  | Ok (Sema.BS_select { where = Some _; _ }) ->
    Alcotest.fail "where bound incorrectly (text col)"
  | Ok _ -> Alcotest.fail "expected BS_select"
  | Error _ -> Alcotest.fail "unexpected error"
;;
    ...
    Ast.S_insert
      ...
      ; values =
          [ [ Ast.E_lit (Ast.L_int 1L)
            ; Ast.E_lit (Ast.L_int 2L)
            ; Ast.E_lit (Ast.L_int 3L)
            ]
          ]
      ; on_conflict = None
      ; returning = []
...
let bind_select_multi_order_by_rejected () =
  ...
  | Ok _ -> ()
  | Error e ->
    Alcotest.failf "unexpected error for multi-column ORDER BY: %a" Sema.pp_error e
;;
...
let bind_update_unknown_table () =
  ...
  let stmt =
    Ast.S_update
      { table = "ghost"
      ; assignments = [ "x", Ast.E_lit (Ast.L_int 1L) ]
      ; where = None
      ; order = []
      ; limit = None
      ; offset = None
...
let bind_update_unknown_col () =
  ...
  let stmt =
    Ast.S_update
      { table = "users"
      ; assignments = [ "bogus", Ast.E_lit (Ast.L_int 1L) ]
      ; where = None
      ; order = []
      ; limit = None
      ; offset = None
...
let bind_update_type_mismatch () =
  ...
  let stmt =
    Ast.S_update
      { table = "users"
      ; assignments = [ "id", Ast.E_lit (Ast.L_text "bad") ]
      ; where = None
      ; order = []
      ; limit = None
      ; offset = None
...
let bind_update_not_null_violation () =
  ...
  let stmt =
    Ast.S_update
      { table = "users"
      ; assignments = [ "id", Ast.E_lit Ast.L_null ]
      ; where = None
      ; order = []
      ; limit = None
      ; offset = None
...
let bind_drop_table_basic () =
  ...
  match bind cat stmt with
  | Ok (Sema.BS_drop_table { name; _ }) -> Alcotest.(check string) "name" "users" name
...
let bind_drop_index_basic () =
  ...
  match bind cat stmt with
  | Ok (Sema.BS_drop_index { name; _ }) -> Alcotest.(check string) "index name" "idx" name
    ...
    Ast.S_select
      ...
      ; proj =
          `Exprs
            [ Ast.E_col "id", None
            ; Ast.E_col "name", None
            ; Ast.E_agg (Ast.Agg_count, None), None
            ]
      ; table = "users"
      ; table_alias = None
      ; joins = []
  ...
  let stmt =
    Ast.S_select
      ...
      ; having =
          Some
            (Ast.E_binop
               (Ast.Gt, Ast.E_agg (Ast.Agg_count, None), Ast.E_lit (Ast.L_int 0L)))
      ; order = []
      ; limit = None
      ; offset = None
      }
...
let bind_select_multi_join_supported () =
  ...
  let cat = make_join_cat () in
  let join_clause tbl =
    { Ast.kind = Ast.Inner; table = tbl; alias = None; on = Ast.E_lit (Ast.L_int 1L) }
  in
  let stmt =
    Ast.S_select
      { distinct = false
    ...
    Ast.S_select
      ...
      ; joins =
          [ { Ast.kind = Ast.Inner
            ...
            ; on =
                Ast.E_binop
                  (Ast.Eq, Ast.E_tbl_col ("orders", "item"), Ast.E_lit (Ast.L_text "x"))
            }
          ]
      ; where = None
      ; group_by = []
    ...
    Ast.S_select
      ...
      ; joins =
          [ { Ast.kind = Ast.Inner
            ; table = "orders"
            ; alias = None
            ; on = Ast.E_binop (Ast.Eq, Ast.E_tbl_col ("users", "id"), Ast.E_col "bogus")
            }
          ]
      ; where = None
      ; group_by = []
...
let bind_select_agg_qual_col_arg () =
  ...
  let stmt =
    Ast.S_select
      { distinct = false
      ; proj =
          `Exprs [ Ast.E_agg (Ast.Agg_sum, Some (Ast.E_tbl_col ("users", "id"))), None ]
      ; table = "users"
      ; table_alias = None
      ; joins = []
      ; where = None
...
let bind_select_agg_qual_col_arg_error () =
  ...
  let stmt =
    Ast.S_select
      { distinct = false
      ; proj =
          `Exprs [ Ast.E_agg (Ast.Agg_sum, Some (Ast.E_tbl_col ("ghost", "id"))), None ]
      ; table = "users"
      ; table_alias = None
      ; joins = []
      ; where = None
...
let bind_select_qual_no_join () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; proj =
          `Exprs
            [ Ast.E_tbl_col ("users", "id"), None; Ast.E_agg (Ast.Agg_count, None), None ]
      ; table = "users"
      ; table_alias = None
      ; joins = []
      ; where = None
...
let bind_select_qual_no_join_unknown_col () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; proj =
          `Exprs
            [ Ast.E_tbl_col ("users", "bogus"), None
            ; Ast.E_agg (Ast.Agg_count, None), None
            ]
      ; table = "users"
      ; table_alias = None
...
let bind_select_qual_no_join_unknown_table () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; proj =
          `Exprs
            [ Ast.E_tbl_col ("ghost", "id"), None; Ast.E_agg (Ast.Agg_count, None), None ]
      ; table = "users"
      ; table_alias = None
      ; joins = []
      ; where = None
...
let bind_select_join_on_ambiguous () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; table_alias = None
      ; joins =
          [ { Ast.kind = Ast.Inner; table = "b"; alias = None; on = Ast.E_col "x" } ]
      ; where = None
      ; group_by = []
      ; having = None
      ; order = []
    ...
    Ast.S_select
      ...
      ; joins =
          [ { Ast.kind = Ast.Inner
            ; table = "orders"
            ; alias = None
            ; on = Ast.E_binop (Ast.Eq, Ast.E_col "bogus", Ast.E_tbl_col ("orders", "uid"))
            }
          ]
      ; where = None
      ; group_by = []
  ...
  let stmt =
    Ast.S_update
      ...
      ; assignments =
          [ ( "id"
            , Ast.E_between
                (Ast.E_col "id", Ast.E_lit (Ast.L_int 1L), Ast.E_lit (Ast.L_int 10L)) )
          ]
      ; where = None
      ; order = []
      ; limit = None
  ...
  let stmt =
    Ast.S_update
      ...
      ; assignments =
          [ ( "id"
            , Ast.E_in
                (Ast.E_col "id", [ Ast.E_lit (Ast.L_int 1L); Ast.E_lit (Ast.L_int 2L) ]) )
          ]
      ; where = None
      ; order = []
      ; limit = None
...
let bind_update_arith_real_mismatch () =
  ...
  let stmt =
    Ast.S_update
      { table = "users"
      ; assignments =
          [ "id", Ast.E_binop (Ast.Add, Ast.E_col "id", Ast.E_lit (Ast.L_real 1.5)) ]
      ; where = None
      ; order = []
      ; limit = None
      ; offset = None
...
let bind_update_concat_type_mismatch () =
  ...
  let stmt =
    Ast.S_update
      { table = "users"
      ; assignments =
          [ "id", Ast.E_binop (Ast.Concat, Ast.E_col "name", Ast.E_lit (Ast.L_text "x")) ]
      ; where = None
      ; order = []
      ; limit = None
      ; offset = None
...
let bind_compound_nested () =
  ...
  let inner =
    Ast.S_compound
      { op = Ast.Union; left = sel; right = sel; order = []; limit = None; offset = None }
  in
  let stmt =
    Ast.S_compound
      { op = Ast.Union_all
...
let bind_returning_params_named () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; where =
          Some
            (Ast.E_binop (Ast.Eq, Ast.E_col "id", Ast.E_param (Ast.Param_name "myval")))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
...
let bind_select_between_hi_error () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; where =
          Some
            (Ast.E_between (Ast.E_col "id", Ast.E_lit (Ast.L_int 1L), Ast.E_col "bogus"))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
...
let bind_select_in_subject_error () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; table_alias = None
      ; joins = []
      ; where = Some (Ast.E_in (Ast.E_col "bogus", [ Ast.E_lit (Ast.L_int 1L) ]))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
...
let bind_select_in_value_error () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; table_alias = None
      ; joins = []
      ; where = Some (Ast.E_in (Ast.E_col "id", [ Ast.E_col "bogus" ]))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
    ...
    Ast.S_select
      ...
      ; joins =
          [ { Ast.kind = Ast.Inner
            ; table = "orders"
            ; alias = None
            ; on = Ast.E_in (Ast.E_tbl_col ("users", "id"), [ Ast.E_col "bogus" ])
            }
          ]
      ; where = None
      ; group_by = []
    ...
    Ast.S_select
      ...
      ; joins =
          [ { Ast.kind = Ast.Inner
            ...
            ; on =
                Ast.E_binop
                  (Ast.Eq, Ast.E_tbl_col ("users", "id"), Ast.E_param Ast.Param_anon)
            }
          ]
      ; where = None
      ; group_by = []
  ...
  let stmt =
    Ast.S_select
      ...
      ; having =
          Some
            (Ast.E_binop
               (Ast.Gt, Ast.E_agg (Ast.Agg_count, None), Ast.E_param Ast.Param_anon))
      ; order = []
      ; limit = None
      ; offset = None
      }
...
let bind_compound_union_ok () =
  ...
  let stmt =
    Ast.S_compound
      { op = Ast.Union; left; right; order = []; limit = None; offset = None }
  in
  match bind cat stmt with
...
let bind_select_like_op () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; joins = []
      ; where =
          Some (Ast.E_binop (Ast.Like, Ast.E_col "name", Ast.E_lit (Ast.L_text "a%")))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
...
let bind_select_glob_op () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; joins = []
      ; where =
          Some (Ast.E_binop (Ast.Glob, Ast.E_col "name", Ast.E_lit (Ast.L_text "a*")))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
...
let bind_select_mod_op () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; table_alias = None
      ; joins = []
      ; where = Some (Ast.E_binop (Ast.Mod, Ast.E_col "id", Ast.E_lit (Ast.L_int 2L)))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
...
let bind_select_bit_and_op () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; table_alias = None
      ; joins = []
      ; where = Some (Ast.E_binop (Ast.Bit_and, Ast.E_col "id", Ast.E_lit (Ast.L_int 1L)))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
...
let bind_select_lshift_op () =
  ...
  let stmt =
    Ast.S_select
      ...
      ; table_alias = None
      ; joins = []
      ; where = Some (Ast.E_binop (Ast.Lshift, Ast.E_col "id", Ast.E_lit (Ast.L_int 1L)))
      ; group_by = []
      ; having = None
      ; order = []
      ; limit = None
  ...
  Alcotest.run
    ...
    ; ( "insert"
      , [ Alcotest.test_case "bind_insert_basic" `Quick bind_insert_basic
        ...
        ; Alcotest.test_case
            "bind_insert_arity_mismatch"
            `Quick
            bind_insert_arity_mismatch
        ; Alcotest.test_case
            "bind_insert_type_mismatch_int_col"
            `Quick
            bind_insert_type_mismatch_int_col
        ; Alcotest.test_case
            "bind_insert_type_mismatch_text_col"
            `Quick
            bind_insert_type_mismatch_text_col
        ; Alcotest.test_case "bind_insert_null_allowed" `Quick bind_insert_null_allowed
        ; Alcotest.test_case
            "bind_insert_second_col_unknown"
            `Quick
            bind_insert_second_col_unknown
        ; Alcotest.test_case
            "bind_insert_not_null_violation"
            `Quick
            bind_insert_not_null_violation
        ] )
    ; ( "select"
      , [ Alcotest.test_case "bind_select_star" `Quick bind_select_star
        ...
        ; Alcotest.test_case
            "bind_select_where_col_eq_lit"
            `Quick
            bind_select_where_col_eq_lit
        ; Alcotest.test_case
            "bind_select_where_lit_eq_col"
            `Quick
            bind_select_where_lit_eq_col
        ; Alcotest.test_case
            "bind_select_where_unknown_col"
            `Quick
            bind_select_where_unknown_col
        ; Alcotest.test_case
            "bind_select_where_text_col"
            `Quick
            bind_select_where_text_col
        ; Alcotest.test_case
            "bind_select_where_right_unknown"
            `Quick
            bind_select_where_right_unknown
        ; Alcotest.test_case
            "bind_select_second_col_unknown"
            `Quick
            bind_select_second_col_unknown
        ; Alcotest.test_case
            "bind_select_multi_order_by_rejected"
            `Quick
            bind_select_multi_order_by_rejected
        ; Alcotest.test_case
            "bind_select_order_unknown_col"
            `Quick
            bind_select_order_unknown_col
        ; Alcotest.test_case
            "bind_select_negative_limit"
            `Quick
            bind_select_negative_limit
        ; Alcotest.test_case
            "bind_select_negative_offset"
            `Quick
            bind_select_negative_offset
        ] )
    ; ( "create-index"
      , [ Alcotest.test_case "bind_create_index_basic" `Quick bind_create_index_basic
        ...
        ; Alcotest.test_case
            "bind_create_index_unknown_table"
            `Quick
            bind_create_index_unknown_table
        ; Alcotest.test_case
            "bind_create_index_unknown_column"
            `Quick
            bind_create_index_unknown_column
        ; Alcotest.test_case
            "bind_create_index_duplicate"
            `Quick
            bind_create_index_duplicate
        ] )
    ; ( "update"
    ...
    ; ( "delete"
      , [ Alcotest.test_case "bind_delete_basic" `Quick bind_delete_basic
        ...
        ; Alcotest.test_case
            "bind_delete_where_unknown_col"
            `Quick
            bind_delete_where_unknown_col
        ] )
    ; ( "subquery-in-dml"
      , [ Alcotest.test_case
            "update_set_subquery_rejected"
            `Quick
            bind_update_set_subquery_rejected
        ; Alcotest.test_case
            "update_where_subquery_rejected"
            `Quick
            bind_update_where_subquery_rejected
        ; Alcotest.test_case
            "update_where_exists_rejected"
            `Quick
            bind_update_where_exists_rejected
        ; Alcotest.test_case
            "delete_where_subquery_rejected"
            `Quick
            bind_delete_where_subquery_rejected
        ; Alcotest.test_case
            "delete_where_exists_rejected"
            `Quick
            bind_delete_where_exists_rejected
        ; Alcotest.test_case
            "insert_returning_subquery_rejected"
            `Quick
            bind_insert_returning_subquery_rejected
        ; Alcotest.test_case
            "update_returning_subquery_rejected"
            `Quick
            bind_update_returning_subquery_rejected
        ; Alcotest.test_case
            "delete_returning_subquery_rejected"
            `Quick
            bind_delete_returning_subquery_rejected
        ] )
    ; ( "drop"
    ...
    ; ( "join"
      , [ Alcotest.test_case
            "bind_select_join_unknown_table"
            `Quick
            bind_select_join_unknown_table
        ; Alcotest.test_case
            "bind_select_join_ambiguous_col"
            `Quick
            bind_select_join_ambiguous_col
        ; Alcotest.test_case
            "bind_select_join_unknown_proj_col"
            `Quick
            bind_select_join_unknown_proj_col
        ; Alcotest.test_case
            "bind_select_join_qualified_col_ok"
            `Quick
            bind_select_join_qualified_col_ok
        ; Alcotest.test_case
            "bind_select_join_qualified_unknown_table"
            `Quick
            bind_select_join_qualified_unknown_table
        ; Alcotest.test_case
            "bind_select_join_unknown_col_unqual"
            `Quick
            bind_select_join_unknown_col_unqual
        ; Alcotest.test_case
            "bind_select_join_agg_in_on_rejected"
            `Quick
            bind_select_join_agg_in_on_rejected
        ; Alcotest.test_case
            "bind_select_join_qualified_col_unknown_col"
            `Quick
            bind_select_join_qualified_col_unknown_col
        ; Alcotest.test_case
            "bind_select_join_where_unknown_col"
            `Quick
            bind_select_join_where_unknown_col
        ; Alcotest.test_case
            "bind_select_multi_join_supported"
            `Quick
            bind_select_multi_join_supported
        ] )
    ; ( "aggregate"
      , [ Alcotest.test_case "bind_select_count_star" `Quick bind_select_count_star
        ...
        ; Alcotest.test_case
            "bind_select_sum_text_col_rejected"
            `Quick
            bind_select_sum_text_col_rejected
        ; Alcotest.test_case
            "bind_select_avg_text_col_rejected"
            `Quick
            bind_select_avg_text_col_rejected
        ; Alcotest.test_case
            "bind_select_group_by_basic"
            `Quick
            bind_select_group_by_basic
        ; Alcotest.test_case
            "bind_select_group_by_unknown_col"
            `Quick
            bind_select_group_by_unknown_col
        ; Alcotest.test_case
        ...
        ; Alcotest.test_case
            ...
            bind_select_col_not_in_group_by_rejected
        ; Alcotest.test_case
            "bind_select_agg_star_in_select_rejected"
            `Quick
            bind_select_agg_star_in_select_rejected
        ; Alcotest.test_case
            "bind_select_agg_unknown_col_arg"
            `Quick
            bind_select_agg_unknown_col_arg
        ; Alcotest.test_case
            "bind_select_agg_in_where_rejected"
            `Quick
            bind_select_agg_in_where_rejected
        ] )
    ; ( "error-values"
      , [ Alcotest.test_case
            "error_already_exists_message"
            `Quick
            error_already_exists_message
        ; Alcotest.test_case
            "error_unknown_table_message"
            `Quick
            error_unknown_table_message
    ...
    ; ( "gap-fill"
      , [ Alcotest.test_case
            "bind_insert_real_lit_type_mismatch"
            `Quick
            bind_insert_real_lit_type_mismatch
        ...
        ; Alcotest.test_case
            "bind_select_tbl_col_unknown_no_join"
            `Quick
            bind_select_tbl_col_unknown_no_join
        ; Alcotest.test_case
            "bind_select_not_unknown_col"
            `Quick
            bind_select_not_unknown_col
        ; Alcotest.test_case
            "bind_select_is_null_unknown_col"
            `Quick
            bind_select_is_null_unknown_col
        ; Alcotest.test_case
            "bind_select_is_not_null_unknown_col"
            `Quick
            bind_select_is_not_null_unknown_col
        ; Alcotest.test_case
            "bind_select_neg_unknown_col"
            `Quick
            bind_select_neg_unknown_col
        ; Alcotest.test_case
            "bind_select_join_unqual_right_only"
            `Quick
            bind_select_join_unqual_right_only
        ; Alcotest.test_case
            "bind_select_join_qual_right_col"
            `Quick
            bind_select_join_qual_right_col
        ; Alcotest.test_case
            "bind_select_join_binop_right_error"
            `Quick
            bind_select_join_binop_right_error
        ; Alcotest.test_case "bind_select_join_on_not" `Quick bind_select_join_on_not
        ; Alcotest.test_case
            "bind_select_join_on_not_error"
            `Quick
            bind_select_join_on_not_error
        ; Alcotest.test_case
            "bind_select_join_on_is_null"
            `Quick
            bind_select_join_on_is_null
        ; Alcotest.test_case
            "bind_select_join_on_is_null_error"
            `Quick
            bind_select_join_on_is_null_error
        ; Alcotest.test_case
            "bind_select_join_on_is_not_null"
            `Quick
            bind_select_join_on_is_not_null
        ; Alcotest.test_case
            "bind_select_join_on_is_not_null_error"
            `Quick
            bind_select_join_on_is_not_null_error
        ; Alcotest.test_case "bind_select_join_on_neg" `Quick bind_select_join_on_neg
        ; Alcotest.test_case
            "bind_select_join_on_neg_error"
            `Quick
            bind_select_join_on_neg_error
        ; Alcotest.test_case
            "bind_select_having_with_plain_col"
            `Quick
            bind_select_having_with_plain_col
        ; Alcotest.test_case
            "bind_select_having_unknown_col"
            `Quick
            bind_select_having_unknown_col
        ; Alcotest.test_case
            "bind_select_having_qual_col"
            `Quick
            bind_select_having_qual_col
        ; Alcotest.test_case
            "bind_select_having_qual_col_error"
            `Quick
            bind_select_having_qual_col_error
        ; Alcotest.test_case
            "bind_select_having_with_not_agg"
            `Quick
            bind_select_having_with_not_agg
        ; Alcotest.test_case
            "bind_select_having_with_neg_agg"
            `Quick
            bind_select_having_with_neg_agg
        ; Alcotest.test_case
            "bind_select_having_with_is_null_agg"
            `Quick
            bind_select_having_with_is_null_agg
        ; Alcotest.test_case
            "bind_select_having_with_is_not_null_agg"
            `Quick
            bind_select_having_with_is_not_null_agg
        ; Alcotest.test_case
            "bind_select_agg_qual_col_arg"
            `Quick
            bind_select_agg_qual_col_arg
        ; Alcotest.test_case
            "bind_select_agg_qual_col_arg_error"
            `Quick
            bind_select_agg_qual_col_arg_error
        ...
        ; Alcotest.test_case
            "bind_select_multi_join_explicit_success"
            `Quick
            bind_select_multi_join_explicit_success
        ; Alcotest.test_case
            "bind_select_join_right_only_col_proj"
            `Quick
            bind_select_join_right_only_col_proj
        ; Alcotest.test_case "bind_select_qual_no_join" `Quick bind_select_qual_no_join
        ; Alcotest.test_case
            "bind_select_qual_no_join_unknown_col"
            `Quick
            bind_select_qual_no_join_unknown_col
        ; Alcotest.test_case
            "bind_select_qual_no_join_unknown_table"
            `Quick
            bind_select_qual_no_join_unknown_table
        ; Alcotest.test_case
            "bind_select_join_qual_left_unknown_col_proj"
            `Quick
            bind_select_join_qual_left_unknown_col_proj
        ; Alcotest.test_case
            "bind_select_join_on_ambiguous"
            `Quick
            bind_select_join_on_ambiguous
        ; Alcotest.test_case
            "bind_select_join_on_unqual_left_only"
            `Quick
            bind_select_join_on_unqual_left_only
        ; Alcotest.test_case
            "bind_select_join_on_qual_left_unknown"
            `Quick
            bind_select_join_on_qual_left_unknown
        ; Alcotest.test_case
            "bind_select_join_binop_left_error"
            `Quick
            bind_select_join_binop_left_error
        ; Alcotest.test_case
            "bind_select_having_binop_right_error"
            `Quick
            bind_select_having_binop_right_error
        ; Alcotest.test_case
            "bind_select_having_agg_unknown_arg"
            `Quick
            bind_select_having_agg_unknown_arg
        ; Alcotest.test_case
            "bind_select_having_agg_with_col_arg"
            `Quick
            bind_select_having_agg_with_col_arg
        ; Alcotest.test_case
            "bind_select_having_agg_with_qual_arg"
            `Quick
            bind_select_having_agg_with_qual_arg
        ; Alcotest.test_case
    ...
    ; ( "agg-proj-errors"
      , [ Alcotest.test_case
            "having_not_unknown_col"
            `Quick
            bind_select_having_not_unknown_col
        ; Alcotest.test_case
            "having_is_null_unknown_col"
            `Quick
            bind_select_having_is_null_unknown_col
        ; Alcotest.test_case
            "having_is_not_null_unknown_col"
            `Quick
            bind_select_having_is_not_null_unknown_col
        ; Alcotest.test_case
            "having_neg_unknown_col"
            `Quick
            bind_select_having_neg_unknown_col
        ] )
    ; ( "txn-stmts"
    ...
    ; ( "bind-expr-extra"
      , [ Alcotest.test_case "bitnot_ok" `Quick bind_select_bitnot_ok
        ...
        ; Alcotest.test_case
            "func_arity_error_where"
            `Quick
            bind_select_func_arity_error_where
        ; Alcotest.test_case "func_ok_where" `Quick bind_select_func_ok_where
        ; Alcotest.test_case
            "func_arg_error_where"
            `Quick
            bind_select_func_arg_error_where
    ...
    ; ( "fts-table"
      , [ Alcotest.test_case "create_fts_ok" `Quick bind_create_fts_table_ok
        ; Alcotest.test_case
            "create_fts_dup_regular"
            `Quick
            bind_create_fts_table_dup_regular
        ] )
    ; ( "pragma"
    ...
    ; ( "infer-type"
      , [ Alcotest.test_case "update_neg_col_ok" `Quick bind_update_neg_col_ok
        ...
        ; Alcotest.test_case
            "update_arith_real_mismatch"
            `Quick
            bind_update_arith_real_mismatch
        ; Alcotest.test_case
            "update_concat_type_mismatch"
            `Quick
            bind_update_concat_type_mismatch
        ; Alcotest.test_case
            "update_func_no_type_check"
            `Quick
            bind_update_func_no_type_check
        ; Alcotest.test_case
            "update_param_no_type_check"
            `Quick
            bind_update_param_no_type_check
        ] )
    ; ( "compound-extra"
