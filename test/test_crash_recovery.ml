...
let tmp_path () =
  incr counter;
  Printf.sprintf "/tmp/sqlocaml_crash_%d_%d_%d.db"
    (Unix.getpid ()) !counter (Random.int 1_000_000)
...
let open_with_fi ~path ~config =
  ...
  let* r =
    Db.open_block
      ~read_page:read ~write_page:write
      ~sync ~resize ~n_pages ~close
  in
  match r with
...
let exec_maybe db sql =
  Lwt.catch
    ...
    (fun exn ->
       ...
       Lwt.return
         (Error (Db.Runtime
                   (Printf.sprintf "exception escaped Db.execute: %s"
  ...
  Lwt_main.run (
    ...
    Lwt.finalize
      (fun () ->
         ...
         match reopen_r with
         | Ok db2 ->
           Lwt.catch (fun () -> Db.close db2) (fun _ -> Lwt.return_unit)
         | Error _ -> Lwt.return_unit)
      (fun () ->
         safe_unlink path;
         Lwt.return_unit)
...
let () =
  ...
  Alcotest.run "crash_recovery" [
    "injection", [
      Alcotest.test_case "injection_does_not_crash" `Quick
        test_injection_does_not_crash;
      Alcotest.test_case "reopen_after_fault" `Quick
        test_reopen_after_fault;
    ]
  ]
