...
let all_rows_int_pairs db =
  ...
  | Ok stream ->
    ...
    List.map (fun row ->
      let id = match row.(0) with
        | Db.V_int n -> Int64.to_int n
        | _ -> -1
      in
      let n = match row.(1) with
        | Db.V_int n -> Int64.to_int n
        | _ -> -1
      in
      (id, n)
...
let assert_model_matches db model iter =
  ...
  if actual <> expected then begin
    ...
    Printf.eprintf "model has %d rows, db has %d rows\n"
    ...
    (match op with
     | 0 ->
       ...
       if not (Hashtbl.mem model k) then begin
         let ok = exec_or_ignore db (Printf.sprintf "INSERT INTO t (id, n) VALUES (%d, %d)" k v) in
...
let test_heap_no_unbounded_growth () =
  ...
  if growth_ratio > 2.0 then
    Alcotest.failf
      "heap grew unboundedly: phase1=%d, baseline=%d, phase2=%d (ratio %.2f)"
      after_phase1 baseline after_phase2 growth_ratio;
  run (Db.close db)
...
let test_rollback_during_soak () =
  ...
  if snapshot <> after then
    Alcotest.fail "rolled-back inserts persisted across soak boundary";
  run (Db.close db)

let () =
  Alcotest.run "soak" [
    "soak", [
      Alcotest.test_case (Printf.sprintf "in-memory (%d iters)" n_iters) `Slow test_soak_mem;
      Alcotest.test_case (Printf.sprintf "file backend (%d iters)" n_iters) `Slow test_soak_file;
      Alcotest.test_case "heap growth bounded across phases" `Slow test_heap_no_unbounded_growth;
