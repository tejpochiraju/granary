...
let mk_col name ty : Row.column = { name; ty; not_null = false; primary_key = false; default = None; check_sql = None; generated_as = None }
...
let test_indexes_for_table () =
  run (
    ...
    let* _ = C.create_index cat ~name:"idx_name"
      ~table:"users" ~columns:["name"] ~unique:true ~expr_flags:[false] ~where_sql:None in
...
let test_create_index_unknown_table () =
  run (
    let store = S.create () in
    let* cat = C.open_ store in
    let* r = C.create_index cat ~name:"idx" ~table:"ghost" ~columns:["x"] ~unique:false ~expr_flags:[false] ~where_sql:None in
    (match r with
     | Error _ -> ()
