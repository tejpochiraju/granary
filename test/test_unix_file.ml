    ...
    (let path = fresh_path () in
     ...
     | Ok t ->
       ...
       (match r2 with
        ...
        | Error (UF.Io msg) -> Alcotest.failf "expected Out_of_bounds, got Io %s" msg);
       (* negative page id *)
...
let pp_error_io_test () =
  ...
  Alcotest.(check bool) "contains message" true (string_contains s "something went wrong")
;;
    ...
    (let path = fresh_path () in
     ...
     | Ok t ->
       ...
       (match rr with
        ...
        | Ok () ->
          Alcotest.(check char)
            "page 0 preserved after grow"
            'P'
            (Cstruct.get_char rbuf 0));
       let* _ = UF.close t in
       cleanup path;
       Lwt.return_unit)
;;
...
let cross_process_lock_test () =
  ...
  | child_pid ->
    ...
    (match result with
     | `Locked -> ()
     | `Success -> Alcotest.fail "expected open to fail while child holds the lock")
;;
...
let () =
  ...
  Alcotest.run
    ...
    ; ( "read_write"
      , [ Alcotest.test_case
            "resize_write_read_roundtrip"
            `Quick
            resize_write_read_roundtrip_test
    ...
    ; "sync", [ Alcotest.test_case "sync_after_write" `Quick sync_after_write_test ]
    ; ( "flock"
    ...
    ; "closed_fd", [ Alcotest.test_case "ops_on_closed_fd" `Quick ops_on_closed_fd_test ]
    ; ( "resize"
    ...
    ; ( "cooperation"
      , [ Alcotest.test_case
            "read_page yields to concurrent timer"
            `Quick
            read_page_yields_to_timer_test
        ] )
    ; "qcheck", qcheck_tests
    ]
;;
