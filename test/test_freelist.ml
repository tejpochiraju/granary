...
(* empty has size = 0 *)
let test_empty_size () = Alcotest.(check int) "empty size = 0" 0 (FL.size FL.empty)
...
let test_pop_oldest_first () =
  ...
  | Some (pid, t') ->
    ...
    Alcotest.(check bool)
      "page freed at 5 not reusable yet"
      true
...
let test_size_tracking () =
  ...
  | Some (_, t3') ->
    ...
    (match FL.pop t3' ~min_safe_txn_id:100L with
     ...
     | Some (_, t3'') -> Alcotest.(check int) "size after pop 2" 1 (FL.size t3''))
;;
...
let test_reusable_count_partial () =
  ...
  Alcotest.(check int) "reusable_count = 2" 2 (FL.reusable_count t ~min_safe_txn_id:3L)
;;
...
let test_pop_returns_correct_page () =
  ...
  | None -> Alcotest.fail "expected Some"
  | Some (pid, _) -> Alcotest.(check int32) "exact page_id" 0xDEADBEEFl pid
;;
  ...
  QCheck.Test.make
    ...
    (fun entries ->
       ...
       let rec compare_pops t1 t2 =
         match
           ( FL.pop t1 ~min_safe_txn_id:Int64.max_int
           , FL.pop t2 ~min_safe_txn_id:Int64.max_int )
         with
         | None, None -> true
         | Some (p1, t1'), Some (p2, t2') -> p1 = p2 && compare_pops t1' t2'
         | _ -> false
       in
       compare_pops t t')
;;
  ...
  QCheck.Test.make
    ...
    (fun entries ->
       ...
       let rec check t_cur =
         match FL.pop t_cur ~min_safe_txn_id:Int64.max_int with
         | None -> true
         | Some (pid, t') -> List.mem pid page_ids && check t'
       in
       check t)
;;
...
let () =
  ...
  Alcotest.run
    ...
    ; ( "serialisation"
      , [ Alcotest.test_case
            "to_list / of_list idempotent"
            `Quick
            test_to_of_list_idempotent
        ] )
    ; ( "reusable_count"
    ...
    ; ( "correctness"
      , [ Alcotest.test_case
            "pop returns correct page_id"
            `Quick
            test_pop_returns_correct_page
        ] )
    ; "qcheck", qcheck_tests
    ]
;;
