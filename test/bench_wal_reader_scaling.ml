...
let setup () =
  ...
  (match run (Db.execute db "CREATE TABLE t (n INTEGER)") with
   | Ok _ -> ()
   | Error _ -> failwith "ddl");
  for i = 0 to 999 do
    match run (Db.execute db (Printf.sprintf "INSERT INTO t VALUES (%d)" i)) with
    | Ok _ -> ()
    | Error _ -> failwith "seed"
  done;
  db
;;
  ...
  let make_reader _ =
    let rec loop i =
      ...
      then Lwt.return ()
        ...
        | Ok stream -> Lwt_stream.to_list stream >>= fun _ -> loop (i + 1)
    in
    loop 0
  in
...
let test_scaling () =
  ...
  Printf.printf
    "serial=%.3fs parallel=%.3fs ratio=%.2f (max %.2f)\n%!"
    t_serial
    t_par
    ratio
    parallel_max;
...
let () =
  Alcotest.run
    ...
    [ ( "bench"
      , [ Alcotest.test_case
            "parallel readers do not regress vs serial"
            `Slow
            test_scaling
        ] )
    ]
