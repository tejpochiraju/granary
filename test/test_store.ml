...
let test_cursor_exhaustion () =
  run
    (let s = S.create () in
     ...
     let _ = S.cursor_next cur in
     (* "a" *)
     let _ = S.cursor_next cur in
     (* "b" *)
     let after = S.cursor_next cur in
     (* exhausted *)
     let again = S.cursor_next cur in
     (* still exhausted *)
     S.cursor_close cur;
     let* () = S.ro_end tx in
...
let cursor_value_after_next () =
  run
    (let s = S.create () in
     ...
     Alcotest.(check bool) "cursor_value None after next" true (S.cursor_value c = None);
     S.cursor_close c;
     S.ro_end tx)
;;
...
let test_many_trees () =
  run
    (let s = S.create () in
     ...
     let* () =
       Lwt_list.iter_s
         (fun i -> S.put tx i (bs "k") (Bytes.of_string (string_of_int i)))
         (List.init 10 Fun.id)
     in
     let* () = S.commit tx in
     let* tx = S.ro_begin s in
     let* results = Lwt_list.map_s (fun i -> S.get tx i (bs "k")) (List.init 10 Fun.id) in
     let* () = S.ro_end tx in
     List.iteri
       (fun i got ->
...
let test_mem_freelist_entries () =
  let s = S.create () in
  Alcotest.(check int) "Mem freelist_entries = []" 0 (List.length (S.freelist_entries s))
;;
    ...
    (fun keys ->
       ...
       Lwt_main.run
         (let s = S.create () in
          let* tx = S.rw_begin s in
          let* () =
            Lwt_list.iter_s (fun k -> S.put tx 0 k (Bytes.of_string "v")) unique_keys
          in
          let* () = S.commit tx in
          let* tx = S.ro_begin s in
          let* cur = S.cursor_open tx 0 in
...
let () =
  let qcheck_tests =
    List.map QCheck_alcotest.to_alcotest [ prop_put_get_roundtrip; prop_cursor_sorted ]
  in
  Alcotest.run
    "store"
    [ ( "put_get"
