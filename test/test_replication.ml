...
let make_frame ~epoch ~frame_idx ~page_id ~is_commit ~page =
  ...
  Replication.{ epoch; frame_idx; page_id; is_commit; page; checksum; source_salt = 1L; source_seed = 2L }
;;
...
let test_apply_single_batch () =
  Lwt_main.run
    (let* _, wal = fresh_wal () in
     ...
     let frames =
       [ make_frame ~epoch:0L ~frame_idx:0 ~page_id:1L ~is_commit:false ~page:(page_with 'A')
       ; make_frame ~epoch:0L ~frame_idx:1 ~page_id:2L ~is_commit:true ~page:(page_with 'B')
       ]
     in
  ...
  Lwt_main.run
    (let* _, wal = fresh_wal () in
     ...
     let frames =
       [ make_frame ~epoch:0L ~frame_idx:0 ~page_id:1L ~is_commit:false ~page:(page_with 'A')
       ; make_frame ~epoch:0L ~frame_idx:1 ~page_id:2L ~is_commit:true ~page:(page_with 'B')
       ; make_frame ~epoch:0L ~frame_idx:2 ~page_id:3L ~is_commit:true ~page:(page_with 'C')
       ]
     in
     let* r = Replication.apply_frames ~wal ~pager frames in
     (match r with
  ...
  Lwt_main.run
    (let* _, wal = fresh_wal () in
     ...
     let frames =
       ...
       ; make_frame ~epoch:0L ~frame_idx:1 ~page_id:2L ~is_commit:false ~page:(page_with 'B')
       ; make_frame ~epoch:0L ~frame_idx:2 ~page_id:3L ~is_commit:false ~page:(page_with 'C')
       ]
     in
     let* r = Replication.apply_frames ~wal ~pager frames in
     (match r with
...
let test_apply_grows_device () =
  Lwt_main.run
    (let* _, wal = fresh_wal () in
     ...
     let frames =
       [ make_frame ~epoch:0L ~frame_idx:0 ~page_id:50L ~is_commit:true ~page:(page_with 'Z')
       ]
     in
     let* r = Replication.apply_frames ~wal ~pager frames in
     (match r with
