...
let contains haystack needle =
  ...
  let rec loop i =
    ...
    else if String.sub h i nn = n then true
    else loop (i + 1)
  in
  loop 0
...
let test_multiline_statement () =
  ...
  in
  let out = run_repl ~input () in
  Alcotest.(check bool) "multiline create+insert+count works"
    true (contains out "2")
...
let test_dot_tables_and_schema () =
  ...
  Alcotest.(check bool) ".schema printed CREATE for alpha"
    true (contains out "CREATE TABLE")

let test_dot_databases_default () =
  let input = ".databases\n.quit\n" in
  let out = run_repl ~input () in
  Alcotest.(check bool) ".databases shows main"
    true (contains out "main")
...
let test_error_does_not_exit () =
  ...
  in
  let out = run_repl ~input () in
  Alcotest.(check bool) "error reported but loop continued"
    true (contains out "Error" && contains out "1")
...
let test_attach_within_repl () =
  ...
  let out = run_repl ~input () in
  (try Unix.unlink path with _ -> ());
  Alcotest.(check bool) "attached body queried"
    true (contains out "attached");
  Alcotest.(check bool) "databases shows aux"
    true (contains out "aux")
...
let () =
  Alcotest.run "repl" [
    "shell", [
      ...
      Alcotest.test_case "errors don't terminate the loop"  `Quick test_error_does_not_exit;
