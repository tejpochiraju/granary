...
let record_clock_skew st =
  if clock_skew_active
  then (
    let idx = st.entry_index in
    st.entry_index <- idx + 1;
    make_nemesis ~nemesis_name:"clock-skew" ~process:(-1) ~index:idx)
  else make_nemesis ~nemesis_name:"no-clock-skew" ~process:(-1) ~index:0
;;
...
let start_lazyfs mount_dir =
  ...
  Unix.sleep 2;
  if pid = 0
  then failwith "lazyfs failed to start"
  else { pid; mount_dir; fifo_path; config_path }
;;
...
let stop_lazyfs ls =
  if ls.pid > 0
  then (
    (try Unix.kill ls.pid Sys.sigterm with
     | _ -> ());
    Unix.sleep 1;
    (try Sys.remove ls.fifo_path with
     | _ -> ());
    (try Sys.remove ls.config_path with
     | _ -> ());
    try
      ignore (Sys.command (Printf.sprintf "fusermount -u %s 2>/dev/null" ls.mount_dir))
    with
    | _ -> ())
;;
