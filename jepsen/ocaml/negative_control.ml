...
let build_dirty_read () =
  ...
  let txn2 = [ Read (0, []) ] in
  [ { typ = Invoke; f = "txn"; value = Txn txn1; process = 0; index = 0; time_ns = t0 }
  ; { typ = Invoke; f = "txn"; value = Txn txn2; process = 1; index = 1; time_ns = t2 }
  ; { typ = Ok
    ; f = "txn"
    ; value = Txn [ Read (0, [ 1 ]) ]
    ; process = 1
...
let build_bank_lost_transfer () =
  ...
  ; (* But the balance read shows total went from 200 to 210 — fabricated! *)
    { typ = Ok
    ; f = "transfer"
    ; value = BankRead [ 0, 90L; 1, 120L ]
    ; (* 90+120=210, should be 200 *)
      process = 0
    ; index = 1
    ; time_ns = t1
    }
...
let build_set_lost_element () =
  ...
  let t5 = 500_000L in
  [ (* Add element 1 *)
    { typ = Invoke; f = "add"; value = SetAdd 1; process = 0; index = 0; time_ns = t0 }
  ; { typ = Ok; f = "add"; value = SetAdd 1; process = 0; index = 1; time_ns = t1 }
  ; (* Add element 2 *)
    { typ = Invoke; f = "add"; value = SetAdd 2; process = 0; index = 2; time_ns = t2 }
  ; { typ = Ok; f = "add"; value = SetAdd 2; process = 0; index = 3; time_ns = t3 }
...
let build_counter_non_monotonic () =
  ...
  [ (* Increment key 0 twice *)
    { typ = Invoke; f = "add"; value = Add (0, 1); process = 0; index = 0; time_ns = t0 }
  ; { typ = Ok
    ; f = "add"
    ; value = Read (0, Some 1)
    ; process = 0
  ...
  ; { typ = Invoke; f = "add"; value = Add (0, 1); process = 0; index = 2; time_ns = t2 }
  ; { typ = Ok
    ; f = "add"
    ; value = Read (0, Some 2)
    ; process = 0
