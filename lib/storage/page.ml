...
let compute_crc buf =
  ...
  for i = 0 to page_size - 1 do
    ...
    let byte =
      if i >= 8 && i <= 11 then 0
      else Char.code (Cstruct.get_char buf i)
    in
    let idx = Int32.to_int (Int32.logand
                              (Int32.logxor !crc (Int32.of_int byte))
                              0xFFl) in
    crc := Int32.logxor
...
let read_header_fields buf =
  ...
  { txn_id; root_page; freelist_page; n_pages_total; schema_version;
...
let branch_entry_at buf ~offset =
  ...
  if offset + 6 > page_size then `End
  else begin
    let key_len = Cstruct.BE.get_uint16 buf offset in
    if offset + 2 + key_len + 4 > page_size then `End
    else begin
...
let leaf_entry_at buf ~offset =
  ...
  if offset + 4 > page_size then `End
  else begin
    let key_len = Cstruct.BE.get_uint16 buf offset in
    if offset + 2 + key_len + 2 > page_size then `End
    else begin
      let val_len = Cstruct.BE.get_uint16 buf (offset + 2 + key_len) in
      if offset + 2 + key_len + 2 + val_len > page_size then `End
      else begin
        let key = Bytes.create key_len in
        Cstruct.blit_to_bytes buf (offset + 2) key 0 key_len;
