...
let rec parse_value st =
  ...
  | Some '{' ->
    ...
    parse_object st
  | Some 't' ->
    if String.length st.s - st.pos >= 4 && String.sub st.s st.pos 4 = "true"
    then (
      st.pos <- st.pos + 4;
      Ok (J_bool true))
    else Error (Printf.sprintf "unexpected token at %d" st.pos)
  | Some 'f' ->
    if String.length st.s - st.pos >= 5 && String.sub st.s st.pos 5 = "false"
    then (
      st.pos <- st.pos + 5;
      Ok (J_bool false))
    else Error (Printf.sprintf "unexpected token at %d" st.pos)
  | Some 'n' ->
    if String.length st.s - st.pos >= 4 && String.sub st.s st.pos 4 = "null"
    then (
      st.pos <- st.pos + 4;
      Ok J_null)
    else Error (Printf.sprintf "unexpected token at %d" st.pos)
              ...
              (match parse_value st with
               ...
               | Ok value ->
                 ...
                 (match peek st with
                  ...
                  | Some '}' ->
                    advance st;
                    Ok (J_object (List.rev ((key, value) :: acc)))
                  | Some c -> Error (Printf.sprintf "expected ',' or '}', got '%c'" c)
  ...
  then Error ("path must start with '$': " ^ path)
    ...
    while (not !err) && !pos < n do
      ...
      | '.' ->
        incr pos;
        let start = !pos in
        while !pos < n && path.[!pos] <> '.' && path.[!pos] <> '[' do
          incr pos
        done;
        if !pos = start
        then err := true
    ...
    if !err then Error ("invalid JSON path: " ^ path) else Ok (List.rev !steps))
;;
     ...
     | J_object kvs ->
       ...
       (match mode with
        | Set ->
          ...
          then
            J_object
              (List.map (fun (k2, v2) -> if k2 = k then k2, new_val else k2, v2) kvs)
          else J_object (kvs @ [ k, new_val ])
        | Insert -> if exists then v else J_object (kvs @ [ k, new_val ])
        | Replace ->
          if not exists
          then v
          else
    ...
    (match v with
     | J_array elems ->
       ...
       then v
         ...
         J_array
           (List.mapi
              (fun j e -> if j = i then path_modify_go mode new_val e rest else e)
              elems)
     | _ -> v)
;;
      ...
      | [ Idx i ] ->
        (match v with
         | J_array elems ->
           let n = List.length elems in
           let i = if i < 0 then n + i else i in
           if i < 0 || i >= n then v else J_array (List.filteri (fun j _ -> j <> i) elems)
         | _ -> v)
      | Key k :: rest ->
        (match v with
         | J_object kvs ->
           (match List.assoc_opt k kvs with
            | Some sub ->
              ...
              J_object
                (List.map (fun (k2, v2) -> if k2 = k then k2, new_sub else k2, v2) kvs)
            | None -> v)
         | _ -> v)
      | Idx i :: rest ->
        (match v with
