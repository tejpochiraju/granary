...
let jd_of_dt dt =
  ...
  let day_frac =
    (float_of_int dt.hour +. (float_of_int dt.min /. 60.0) +. (dt.sec /. 3600.0)) /. 24.0
  in
  base +. day_frac
;;
...
let parse_date_part s =
  try
    ...
    then None
      ...
      if s.[4] <> '-' || s.[7] <> '-' then None else Some (y, m, d))
  with
...
let parse ?(now : (unit -> float) option) ts =
  ...
  then (
    ...
    | Some (y, m, d) ->
      ...
      (match rest_start with
       | None -> Ok { year = y; month = m; day = d; hour = 0; min = 0; sec = 0.0 }
       | Some i ->
  ...
  else if String.length ts >= 5 && ts.[2] = ':'
  then (
    match parse_time_part ts with
    | None -> Error ("invalid time: " ^ ts)
    | Some (h, mi, sec) -> Ok { year = 2000; month = 1; day = 1; hour = h; min = mi; sec })
  else (
    ...
    | Some f ->
      ...
      then
        if String.contains ts '.' && f > 1000.0
        then Ok (dt_of_jd f)
        else Ok (dt_of_jd (jd_of_unix f))
      else Error ("invalid numeric time: " ^ ts))
;;
...
let to_date dt = pad4 dt.year ^ "-" ^ pad2 dt.month ^ "-" ^ pad2 dt.day
let to_time dt = pad2 dt.hour ^ ":" ^ pad2 dt.min ^ ":" ^ pad2 (int_of_float dt.sec)
...
let day_of_year y m d =
  let rec go mo acc =
    if mo >= m then acc + d else go (mo + 1) (acc + days_in_month y mo)
  in
  go 1 0
;;
  ...
  while !i < n do
    ...
    then (
      ...
      (match fmt.[!i] with
       ...
       | 'j' -> Buffer.add_string buf (pad3 (day_of_year dt.year dt.month dt.day))
       | 's' -> Buffer.add_string buf (Int64.to_string (to_unixepoch dt))
       | '%' -> Buffer.add_char buf '%'
       | c ->
         Buffer.add_char buf '%';
