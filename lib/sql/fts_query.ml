...
type raw_tok = RT_word of string | RT_phrase of string | RT_or | RT_not_word of string
...
let lex query =
  ...
  while !i < n do
    ...
    if !i >= n then ()
      ...
      toks := (if String.uppercase_ascii word = "OR" then RT_or else RT_word word) :: !toks
    end
  done;
  List.rev !toks
...
let parse query =
  ...
  let groups_rev = List.fold_left (fun (cur, groups) tok ->
    match tok with
    | RT_or -> ([], cur :: groups)
    | t     -> (t :: cur, groups)) ([], []) raw in
