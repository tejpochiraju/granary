(* Compile-time proof that the granary core can be consumed by a library that
   ...
   platform-agnostic entry points; its dune stanza lists [granary] (+ [lwt])
...
let _open_block = Granary.Db.open_block
let probe () : Granary.Db.t Lwt.t = Granary.Db.open_in_memory ()
