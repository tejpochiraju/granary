(** Offline repair/recovery CLI: salvage a damaged database into a clean file.
    ...
    Usage: sqlocaml_recover --in <damaged.db> --out <clean.db>
...
let main () : unit Lwt.t =
  ...
  let damaged_file =
    ...
    | Ok f -> f
    | Error e ->
      Format.eprintf "Cannot open '%s': %a\n%!" in_path Sqlocaml_unix.Unix_file.pp_error e;
      exit 1
  in
  ...
  let read_page (page_id : int64) : Cstruct.t option Lwt.t =
    let page_size = Sqlocaml_unix.Unix_file.page_size damaged_file in
    let buf = Cstruct.create page_size in
    let* result = Sqlocaml_unix.Unix_file.read_page damaged_file ~page_id buf in
    match result with
  ...
  (* open_source callback: opens the damaged file via the Store layer
     ...
     during cleanup. *)
  let source_store_ref : Sqlocaml_store.Store.t option ref = ref None in
  let open_source () =
    let* store_result = Sqlocaml_unix.Store.open_file ~path:in_path () in
    match store_result with
    | Error e ->
      let msg = Format.asprintf "%a" Sqlocaml_store.Store.pp_error e in
  ...
  let output_store_ref : Sqlocaml_store.Store.t option ref = ref None in
  let write_to rows =
    let* store =
      match !output_store_ref with
      | Some s -> Lwt.return (Ok s)
      | None ->
        let* result = Sqlocaml_unix.Store.open_file ~path:out_path () in
        (match result with
         | Error e ->
           let msg = Format.asprintf "%a" Sqlocaml_store.Store.pp_error e in
           Lwt.return (Error msg)
